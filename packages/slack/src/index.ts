...
console.log("🚀 Starting openbmw server...")
const openbmw = await createOpencode({
  port: 0,
})
...
void (async () => {
  const events = await openbmw.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new openbmw session...")
    const { client, server } = openbmw
    ...
    console.log("✅ Created openbmw session:", createResult.data.id)
  ...
  console.log("📝 Sending to openbmw:", message.text)
