...
`~/.local/share/openbmw/auth.json` 中。
...
1. 在 TUI 中执行 `/connect` 命令，选择 openbmw，然后前往 [opencode.ai/auth](https://opencode.ai/auth)。
...
你可以通过 [Atomic Chat](https://atomic.chat) 配置 openbmw 以使用本地模型。Atomic Chat 是一款桌面应用程序，它在 OpenAI 兼容的 API 服务器后面运行本地 LLM（默认端点 `http://127.0.0.1:1337/v1`）。
...
:::tip
Ollama 可以自动为 OpenCode 进行配置。详见 [Ollama 集成文档](https://docs.ollama.com/integrations/openbmw)。
:::
...
1. **检查认证设置**：运行 `openbmw auth list` 查看该提供商的凭据是否已添加到配置中。
