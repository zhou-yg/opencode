...
OpenCode 可以与你的 GitHub 工作流集成。在评论中提及 `/openbmw` 或 `/oc`，OpenCode 就会在你的 GitHub Actions 运行器中执行任务。
...
| 事件类型                      | 触发方式                     | 详情                                                                                      |
| ----------------------------- | ---------------------------- | ----------------------------------------------------------------------------------------- |
| `issue_comment`               | 在 Issue 或 PR 上发表评论    | 在评论中提及 `/openbmw` 或 `/oc`。OpenCode 会读取上下文，并可创建分支、提交 PR 或回复。  |
| `pull_request_review_comment` | 在 PR 中对特定代码行发表评论 | 在代码审查时提及 `/openbmw` 或 `/oc`。OpenCode 会接收文件路径、行号和 diff 上下文。      |
