...
- `.openbmw/plugins/` - 项目级插件
- `~/.config/openbmw/plugins/` - 全局插件
...
**npm 插件**在启动时使用 Bun 自动安装。包及其依赖项会缓存在 `~/.cache/openbmw/node_modules/` 中。
...
1. 全局配置 (`~/.config/openbmw/opencode.json`)
2. 项目配置 (`opencode.json`)
3. 全局插件目录 (`~/.config/openbmw/plugins/`)
4. 项目插件目录 (`.openbmw/plugins/`)
