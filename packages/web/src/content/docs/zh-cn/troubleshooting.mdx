...
- **Windows**: 按 `WIN+R` 并粘贴 `%USERPROFILE%\.local\share\openbmw\log`

日志文件以时间戳命名（例如 `2025-01-09T123456.log`），并保留最近的 10 个日志文件。

你可以通过 `--log-level` 命令行选项设置日志级别以获取更详细的调试信息。例如：`openbmw --log-level DEBUG`。
...
- **Windows**: 按 `WIN+R` 并粘贴 `%USERPROFILE%\.local\share\openbmw`
...
- **macOS/Linux**（旧版安装）: `~/.local/share/openbmw/opencode.jsonc`
- **Windows**: 按 `WIN+R` 并粘贴 `%USERPROFILE%\.config\openbmw\opencode.jsonc`
...
- **全局插件**
  ...
  - **Windows**: 按 `WIN+R` 并粘贴 `%USERPROFILE%\.config\openbmw\plugins`
- **项目插件**（仅当你使用了项目级配置时）
...
- **macOS**: Finder -> `Cmd+Shift+G` -> 粘贴 `~/.cache/openbmw`
- **Linux**: 删除 `~/.cache/openbmw`（或运行 `rm -rf ~/.cache/openbmw`）
- **Windows**: 按 `WIN+R` 并粘贴 `%USERPROFILE%\.cache\openbmw`
...
3. 使用 `openbmw upgrade` 确保你使用的是最新版本
...
要查看你有权访问哪些模型，请运行 `openbmw models`
...
2. 如果问题仍然存在，请尝试清除已存储的配置：
   ...
   在 Windows 上，按 `WIN+R` 并删除：`%USERPROFILE%\.local\share\openbmw`
...
1. 清除提供商包缓存：
   ...
   在 Windows 上，按 `WIN+R` 并删除：`%USERPROFILE%\.cache\openbmw`
