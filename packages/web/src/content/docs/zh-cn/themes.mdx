...
默认情况下，OpenCode 使用我们自己的 `openbmw` 主题。
...
1. **内置主题** — 嵌入在二进制文件中
2. **用户配置目录** — 定义在 `~/.config/openbmw/themes/*.json` 或 `$XDG_CONFIG_HOME/openbmw/themes/*.json`
3. **项目根目录** — 定义在 `<project-root>/.openbmw/themes/*.json`
4. **当前工作目录** — 定义在 `./.openbmw/themes/*.json`
