...
| <nobr><code>{"--username"}</code></nobr> | `-u` | 基本认证用户名（默认使用 `OPENCODE_SERVER_USERNAME` 或 `openbmw`） |
...
OpenCode 基于 [Models.dev](https://models.dev) 的提供商列表运行，因此您可以使用 `openbmw auth login` 为任何想要使用的提供商配置 API 密钥。密钥存储在 `~/.local/share/openbmw/auth.json` 中。
...
您还可以连接到正在运行的 `openbmw serve` 实例，以避免每次运行时 MCP 服务器的冷启动时间：
...
| <nobr><code>{"--attach"}</code></nobr>   |      | 连接到正在运行的 openbmw 服务器（例如 http://localhost:4096）      |
| <nobr><code>{"--password"}</code></nobr> | `-p` | 基本认证密码（默认使用 `OPENCODE_SERVER_PASSWORD`）                 |
| <nobr><code>{"--username"}</code></nobr> | `-u` | 基本认证用户名（默认使用 `OPENCODE_SERVER_USERNAME` 或 `openbmw`） |
...
此命令启动一个 HTTP 服务器，提供对 OpenCode 功能的 API 访问，无需 TUI 界面。设置 `OPENCODE_SERVER_PASSWORD` 可启用 HTTP 基本认证（用户名默认为 `openbmw`）。
...
此命令启动一个 HTTP 服务器并打开浏览器，通过 Web 界面访问 OpenCode。设置 `OPENCODE_SERVER_PASSWORD` 可启用 HTTP 基本认证（用户名默认为 `openbmw`）。
...
| `OPENCODE_SERVER_USERNAME`            | string  | 覆盖基本认证用户名（默认为 `openbmw`） |
