...
1. **在 WSL 终端中运行 `openbmw web`**，而非在 PowerShell 中运行：
...
从 WSL 中运行 `openbmw web` 可确保正确的文件系统访问和终端集成，同时仍可通过 Windows 浏览器进行访问。
...
- OpenCode 的配置和会话数据存储在 WSL 环境中的 `~/.local/share/openbmw/`
