...
在你的 OpenCode 配置中，[模型 ID](/docs/config/#models) 使用 `openbmw/<model-id>` 格式。例如，对于 GPT 5.5，你需要在配置中使用 `openbmw/gpt-5.5`。
...
- Ling 2.6 Flash Free：在免费期间，收集的数据可能会被用于改进模型。
- Hy3 Preview Free：在免费期间，收集的数据可能会被用于改进模型。
