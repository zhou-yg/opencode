...
:::tip[Windows Users]
For the best experience, run `openbmw web` from [WSL](/docs/windows-wsl) rather than PowerShell. This ensures proper file system access and terminal integration.
:::
...
The username defaults to `openbmw` but can be changed with `OPENCODE_SERVER_USERNAME`.
