...
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\openbmw\log`

Log files are named with timestamps (e.g., `2025-01-09T123456.log`) and the most recent 10 log files are kept.

You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `openbmw --log-level DEBUG`.

---

## Storage

openbmw stores session data and other application data on disk at:

- **macOS/Linux**: `~/.local/share/openbmw/`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\openbmw`
...
- **macOS/Linux**: `~/.config/openbmw/opencode.jsonc` (or `~/.config/openbmw/opencode.json`)
- **macOS/Linux** (older installs): `~/.local/share/openbmw/opencode.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\openbmw\opencode.jsonc`
...
- **Global plugins**
  ...
  - **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\openbmw\plugins`
...
- **Linux**: delete `~/.cache/openbmw` (or run `rm -rf ~/.cache/openbmw`)
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.cache\openbmw`
...
3. Ensure you have the latest version with `openbmw upgrade`
...
To figure out what models you have access to, run `openbmw models`
...
2. If the issue persists, try clearing your stored configuration:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.local\share\openbmw`
...
If you encounter API call errors, this may be due to outdated provider packages. openbmw dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
1. Clear the provider package cache:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.cache\openbmw`

2. Restart openbmw to reinstall the latest provider packages

This will force openbmw to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
openbmw will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
