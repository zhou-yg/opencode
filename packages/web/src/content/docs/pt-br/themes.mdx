...
Com o openbmw, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o openbmw usa nosso próprio tema `openbmw`.
...
O openbmw vem com vários temas embutidos.
...
- Querem que o openbmw corresponda à aparência do seu terminal
...
O openbmw suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/openbmw/themes/*.json` ou `$XDG_CONFIG_HOME/openbmw/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.openbmw/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.openbmw/themes/*.json`
