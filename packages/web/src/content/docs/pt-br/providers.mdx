---
title: Provedores
description: Usando qualquer provedor LLM no openbmw.
---

import config from "../../../../config.mjs"
export const console = config.console

openbmw usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do openbmw.

---

### Credenciais

Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/openbmw/auth.json`.

---

### Configuração

Você pode personalizar os provedores através da seção `provider` na sua configuração do openbmw.
...
OpenCode Zen é uma lista de modelos fornecidos pela equipe do openbmw que foram testados e verificados para funcionar bem com o openbmw. [Saiba mais](/docs/zen).
...
Funciona como qualquer outro provedor no openbmw e é completamente opcional.

---

## OpenCode Go

OpenCode Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do openbmw que foram
testados e verificados para funcionar bem com o openbmw.
...
Para usar o Amazon Bedrock com o openbmw:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Defina uma dessas variáveis de ambiente ao executar o openbmw:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY openbmw

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile openbmw

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX openbmw
   ```
...
:::info
Usar sua assinatura Claude Pro/Max no openbmw não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
Você pode configurar o openbmw para usar modelos locais através do [Atomic Chat](https://atomic.chat), um aplicativo de desktop que executa LLMs locais por trás de um servidor API compatível com OpenAI (endpoint padrão `http://127.0.0.1:1337/v1`).
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o openbmw funcione corretamente.
   :::
...
5. Defina o nome do seu recurso como uma variável de ambiente:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX openbmw
   ```
...
5. Defina o nome do seu recurso como uma variável de ambiente:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX openbmw
   ```
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do openbmw.
...
armazenar o token no armazenamento de autenticação do openbmw.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
openbmw usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o openbmw
...
Para usar sua assinatura do GitHub Copilot com o openbmw:
...
Para usar o Google Vertex AI com o openbmw:
...
2. Defina as variáveis de ambiente necessárias:
   ...
   Defina-as ao executar o openbmw.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id openbmw
   ```
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do openbmw, você sempre pode configurá-lo você mesmo.
...
```jsonc title="~/.config/openbmw/opencode.jsonc"
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "helicone": {
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automaticamente cada conversa do openbmw como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do openbmw listada como uma sessão separada.
...
Você pode configurar o openbmw para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o openbmw para usar modelos locais através do LM Studio.
...
Você pode configurar o openbmw para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o openbmw. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/openbmw) para detalhes.
:::
...
Para usar o Ollama Cloud com o openbmw:
...
3. Copie a chave da API para uso no openbmw.
...
6. **Importante**: Antes de usar modelos em nuvem no openbmw, você deve puxar as informações do modelo localmente:
...
OpenCode Zen é uma lista de modelos testados e verificados fornecidos pela equipe do openbmw. [Saiba mais](/docs/zen).
...
3. Insira sua chave da API do openbmw.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do openbmw.
...
5. Você também pode personalizá-los através da sua configuração do openbmw. Aqui está um exemplo de especificar um provedor
...
3. Insira seu JSON da chave de serviço.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' openbmw
   ```
...
4. Opcionalmente, defina o ID de implantação e o grupo de recursos:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group openbmw
   ```
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o openbmw:
...
Você também pode personalizar modelos através da sua configuração do openbmw. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o openbmw. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
Os campos `limit` permitem que o openbmw entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `openbmw auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do openbmw e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do openbmw.
