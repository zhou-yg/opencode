---
title: CLI
description: Opções e comandos da CLI do openbmw.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

A CLI do openbmw, por padrão, inicia o [TUI](/docs/tui) quando executada sem argumentos.

```bash
openbmw
```

Mas também aceita comandos conforme documentado nesta página. Isso permite que você interaja com o openbmw programaticamente.
...
Inicie a interface de usuário do terminal do openbmw.
...
A CLI do openbmw também possui os seguintes comandos.

---

### agent

Gerencie agentes para o openbmw.
...
Anexe um terminal a um servidor backend do openbmw já em execução, iniciado via comandos `serve` ou `web`.

```bash
openbmw attach [url]
```

Isso permite usar o TUI com um backend openbmw remoto. Por exemplo:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `openbmw`) |
...
O openbmw é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `openbmw auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/openbmw/auth.json`.

```bash
openbmw auth login
```

Quando o openbmw é iniciado, ele carrega os provedores do arquivo de credenciais. E se houver chaves definidas em seus ambientes ou em um arquivo `.env` em seu projeto.
...
Use a flag `--refresh` para atualizar a lista de modelos em cache. Isso é útil quando novos modelos foram adicionados a um provedor e você deseja vê-los no openbmw.
...
Execute o openbmw em modo não interativo passando um prompt diretamente.
...
```bash "openbmw run"
openbmw run Explique o uso de context em Go
```

Você também pode se anexar a uma instância em execução do `openbmw serve` para evitar tempos de inicialização a frio do servidor MCP em cada execução:
...
openbmw run --attach http://localhost:4096 "Explique async/await em JavaScript"
```
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Anexe a um servidor openbmw em execução (por exemplo, http://localhost:4096)     |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Senha de autenticação básica (padrão: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `openbmw`) |
...
Inicie um servidor openbmw sem cabeça para acesso à API. Confira a [documentação do servidor](/docs/server) para a interface HTTP completa.

```bash
openbmw serve
```

Isso inicia um servidor HTTP que fornece acesso à funcionalidade do openbmw sem a interface TUI. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `openbmw`).
...
Liste todas as sessões do openbmw.
...
Mostre o uso de tokens e estatísticas de custo para suas sessões do openbmw.
...
Importe dados da sessão de um arquivo JSON ou URL de compartilhamento do openbmw.

```bash
openbmw import <file>
```

Você pode importar de um arquivo local ou de uma URL de compartilhamento do openbmw.
...
Inicie um servidor openbmw sem cabeça com uma interface web.

```bash
openbmw web
```

Isso inicia um servidor HTTP e abre um navegador para acessar o openbmw através de uma interface web. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `openbmw`).
...
Desinstale o openbmw e remova todos os arquivos relacionados.
...
Atualiza o openbmw para a versão mais recente ou uma versão específica.
...
A CLI do openbmw aceita as seguintes flags globais.
...
O openbmw pode ser configurado usando variáveis de ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `openbmw`) |
