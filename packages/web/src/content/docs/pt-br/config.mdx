---
title: Configuração
description: Usando a configuração JSON do openbmw.
---

Você pode configurar o openbmw usando um arquivo de configuração JSON.

---

## Formato

O openbmw suporta os formatos **JSON** e **JSONC** (JSON com Comentários).
...
1. **Configuração remota** (de `.well-known/openbmw`) - padrões organizacionais
2. **Configuração global** (`~/.config/openbmw/opencode\.json`) - preferências do usuário
...
5. **Diretórios `.openbmw`** - agentes, comandos, plugins
...
:::note
Os diretórios `.openbmw` e `~/.config/openbmw` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
As organizações podem fornecer configuração padrão através do endpoint `.well-known/openbmw`. Isso é buscado automaticamente quando você se autentica com um provedor que o suporta.
...
Coloque sua configuração global do openbmw em `~/.config/openbmw/opencode\.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.

Para configurações específicas do TUI, use `~/.config/openbmw/tui.json`.
...
Quando o openbmw é iniciado, ele procura um arquivo de configuração no diretório atual ou sobe até o diretório Git mais próximo.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `OPENCODE_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.openbmw`, e deve seguir a mesma estrutura.
...
O diretório personalizado é carregado após a configuração global e os diretórios `.openbmw`, então ele **pode substituir** suas configurações.
...
Você pode configurar as configurações do servidor para os comandos `openbmw serve` e `openbmw web` através da opção `server`.
...
- `mdns` - Habilitar descoberta de serviço mDNS. Isso permite que outros dispositivos na rede descubram seu servidor openbmw.
...
Você pode configurar os provedores e modelos que deseja usar em sua configuração do openbmw através das opções `provider`, `model` e `small_model`.
...
A opção `small_model` configura um modelo separado para tarefas leves, como geração de títulos. Por padrão, o openbmw tenta usar um modelo mais barato se um estiver disponível do seu provedor, caso contrário, ele recua para seu modelo principal.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/openbmw/agents/` ou `.openbmw/agents/`. [Saiba mais aqui](/docs/agents).
...
O agente padrão deve ser um agente primário (não um subagente). Isso pode ser um agente embutido como `"build"` ou `"plan"`, ou um [agente personalizado](/docs/agents) que você definiu. Se o agente especificado não existir ou for um subagente, o openbmw recuará para `"build"` com um aviso.

Essa configuração se aplica a todas as interfaces: TUI, CLI (`openbmw run`), aplicativo desktop e GitHub Action.
...
Você também pode definir comandos usando arquivos markdown em `~/.config/openbmw/commands/` ou `.openbmw/commands/`. [Saiba mais aqui](/docs/commands).
...
O openbmw fará o download automaticamente de quaisquer novas atualizações quando for iniciado. Você pode desabilitar isso com a opção `autoupdate`.
...
Por padrão, o openbmw **permite todas as operações** sem exigir aprovação explícita. Você pode mudar isso usando a opção `permission`.
...
[Plugins](/docs/plugins) estendem o openbmw com ferramentas, hooks e integrações personalizadas.

Coloque arquivos de plugin em `.openbmw/plugins/` ou `~/.config/openbmw/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
...
Isso é útil quando você deseja restringir o openbmw para usar apenas provedores específicos, em vez de desabilitá-los um a um.
