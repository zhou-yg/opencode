---
title: Plugins
description: Escreva seus próprios plugins para estender o openbmw.
---

Plugins permitem que você estenda o openbmw conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do openbmw.
...
- `.openbmw/plugins/` - Plugins em nível de projeto
- `~/.config/openbmw/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/openbmw/node_modules/`.
...
1. Configuração global (`~/.config/openbmw/opencode\.json`)
2. Configuração do projeto (`opencode\.json`)
3. Diretório de plugins global (`~/.config/openbmw/plugins/`)
4. Diretório de plugins do projeto (`.openbmw/plugins/`)
...
```json title=".openbmw/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
O openbmw executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.

```ts title=".openbmw/plugins/my-plugin.ts"
...
```js title=".openbmw/plugins/example.js"
...
- `client`: Um cliente SDK do openbmw para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o openbmw.
...
```js title=".openbmw/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "openbmw"'`
      }
    },
  }
}
...
:::note
Se você estiver usando o aplicativo desktop openbmw, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o openbmw de ler arquivos `.env`:

```javascript title=".openbmw/plugins/env-protection.js"
...
```javascript title=".openbmw/plugins/inject-env.js"
...
Plugins também podem adicionar ferramentas personalizadas ao openbmw:

```ts title=".openbmw/plugins/custom-tools.ts"
...
O helper `tool` cria uma ferramenta personalizada que o openbmw pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o openbmw junto com as ferramentas integradas.
...
```ts title=".openbmw/plugins/compaction.ts"
import type { Plugin } from "@opencode-ai/plugin"
...
```ts title=".openbmw/plugins/custom-compaction.ts"
import type { Plugin } from "@opencode-ai/plugin"
