---
title: Web
description: Usando o openbmw no seu navegador.
---

O openbmw pode ser executado como uma aplicação web no seu navegador, proporcionando a mesma poderosa experiência de codificação com IA sem precisar de um terminal.

![openbmw Web - Nova Sessão](../../../assets/web/web-homepage-new-session.png)
...
Isso inicia um servidor local em `127.0.0.1` com uma porta aleatória disponível e abre automaticamente o openbmw no seu navegador padrão.
...
Para a melhor experiência, execute `openbmw web` a partir do [WSL](/docs/windows-wsl) em vez do PowerShell. Isso garante acesso adequado ao sistema de arquivos e integração com o terminal.
:::
...
Por padrão, o openbmw escolhe uma porta disponível. Você pode especificar uma porta:
...
Por padrão, o servidor se vincula a `127.0.0.1` (apenas localhost). Para tornar o openbmw acessível na sua rede:

```bash
openbmw web --hostname 0.0.0.0
```

Ao usar `0.0.0.0`, o openbmw exibirá endereços locais e de rede:
...
O nome de usuário padrão é `openbmw`, mas pode ser alterado com `OPENCODE_SERVER_USERNAME`.

---

## Usando a Interface Web

Uma vez iniciada, a interface web fornece acesso às suas sessões do openbmw.

### Sessões

Visualize e gerencie suas sessões a partir da página inicial. Você pode ver sessões ativas e iniciar novas.

![openbmw Web - Sessão Ativa](../../../assets/web/web-homepage-active-session.png)

### Status do Servidor

Clique em "Ver Servidores" para visualizar os servidores conectados e seu status.

![openbmw Web - Ver Servidores](../../../assets/web/web-homepage-see-servers.png)
