---
title: GitLab
description: Use o openbmw em problemas e solicitações de mesclagem do GitLab.
---

O openbmw se integra ao seu fluxo de trabalho do GitLab através do seu pipeline CI/CD do GitLab ou com o GitLab Duo.

Em ambos os casos, o openbmw será executado em seus runners do GitLab.

---

## GitLab CI

O openbmw funciona em um pipeline regular do GitLab. Você pode integrá-lo a um pipeline como um [componente CI](https://docs.gitlab.com/ee/ci/components/)

Aqui estamos usando um componente CI/CD criado pela comunidade para openbmw — [nagyv/gitlab-openbmw](https://gitlab.com/nagyv/gitlab-openbmw).

---

### Recursos

- **Use configuração personalizada por trabalho**: Configure o openbmw com um diretório de configuração personalizado, por exemplo `./config/#custom-directory` para habilitar ou desabilitar funcionalidades por invocação do openbmw.
- **Configuração mínima**: O componente CI configura o openbmw em segundo plano, você só precisa criar a configuração do openbmw e o prompt inicial.
...
1. Armazene seu JSON de autenticação do openbmw como variáveis de ambiente do tipo File em **Configurações** > **CI/CD** > **Variáveis**. Certifique-se de marcá-las como "Mascaradas e ocultas".
...
Para mais entradas e casos de uso [consulte a documentação](https://gitlab.com/explore/catalog/nagyv/gitlab-openbmw) deste componente.

---

## GitLab Duo

O openbmw se integra ao seu fluxo de trabalho do GitLab.
Mencione `@opencode` em um comentário, e o openbmw executará tarefas dentro do seu pipeline CI do GitLab.
...
- **Corrigir e implementar**: Peça ao openbmw para corrigir um problema ou implementar uma funcionalidade.
  Ele criará um novo branch e abrirá uma solicitação de mesclagem com as alterações.
- **Seguro**: O openbmw é executado em seus runners do GitLab.

---

### Configuração

O openbmw é executado em seu pipeline CI/CD do GitLab, aqui está o que você precisará para configurá-lo:
...
Aqui estão alguns exemplos de como você pode usar o openbmw no GitLab.
...
- **Explicar um problema**
  ...
  O openbmw lerá o problema e responderá com uma explicação clara.
...
- **Corrigir um problema**
  ...
  O openbmw criará um novo branch, implementará as alterações e abrirá uma solicitação de mesclagem com as alterações.
...
- **Revisar solicitações de mesclagem**
  ...
  O openbmw revisará a solicitação de mesclagem e fornecerá feedback.
