...
Existem dois tipos de agentes no openbmw; agentes primários e subagentes.
...
openbmw vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
openbmw vem com dois subagentes integrados, **General** e **Explore**. Vamos ver isso abaixo.

---

## Integrados

openbmw vem com dois agentes primários integrados e dois subagentes integrados.
...
- Global: `~/.config/openbmw/agents/`
- Por projeto: `.openbmw/agents/`

```markdown title="~/.config/openbmw/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o openbmw usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do openbmw quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do openbmw usa o formato `provider/model-id`. Por exemplo, se você estiver usando [OpenCode Zen](/docs/zen), você usaria `openbmw/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Execute `openbmw models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/openbmw).
:::
...
```markdown title="~/.config/openbmw/agents/docs-writer.md"
---
...
```markdown title="~/.config/openbmw/agents/security-auditor.md"
---
