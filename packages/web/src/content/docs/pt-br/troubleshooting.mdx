...
Para depurar problemas com o openbmw, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\openbmw\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `openbmw --log-level DEBUG`.

---

## Armazenamento

O openbmw armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/openbmw/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\openbmw`
...
O openbmw Desktop executa um servidor openbmw local (o sidecar `opencode-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- **macOS/Linux**: `~/.config/openbmw/opencode.jsonc` (ou `~/.config/openbmw/opencode.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/openbmw/opencode.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\openbmw\opencode.jsonc`
...
O openbmw também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/openbmw/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\openbmw\plugins`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o openbmw possa reconstruí-lo.

1. Saia completamente do openbmw Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/openbmw`
- **Linux**: exclua `~/.cache/openbmw` (ou execute `rm -rf ~/.cache/openbmw`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\openbmw`

3. Reinicie o openbmw Desktop.

---

### Corrigindo problemas de conexão com o servidor

O openbmw Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
No Windows, o openbmw Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do openbmw.

---

### Notificações não aparecendo

O openbmw Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o openbmw nas configurações do seu sistema operacional, e
...
1. Saia do openbmw Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo openbmw Desktop):
...
Se você estiver enfrentando problemas com o openbmw:
...
Para descobrir quais modelos você tem acesso, execute `openbmw models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\openbmw`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O openbmw instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\openbmw`

2. Reinicie o openbmw para reinstalar os pacotes de provedores mais recentes

Isso forçará o openbmw a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O openbmw detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
