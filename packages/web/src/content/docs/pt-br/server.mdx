---
title: Servidor
description: Interaja com o servidor openbmw via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `openbmw serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente openbmw pode usar.
...
```bash
openbmw serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
openbmw serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `OPENCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `openbmw`, ou defina `OPENCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `openbmw serve` quanto ao `openbmw web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password openbmw serve
```
...
Quando você executa `openbmw`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor openbmw para interagir com o openbmw programaticamente.
:::

Essa arquitetura permite que o openbmw suporte múltiplos clientes e permite que você interaja com o openbmw programaticamente.

Você pode executar `openbmw serve` para iniciar um servidor autônomo. Se você tiver o TUI do openbmw em execução, `openbmw serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do openbmw [IDE](/docs/ide).
...
O servidor openbmw expõe as seguintes APIs.
