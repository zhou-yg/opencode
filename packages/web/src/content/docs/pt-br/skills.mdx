...
As habilidades do agente permitem que o openbmw descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O openbmw pesquisa nesses locais:

- Configuração do projeto: `.openbmw/skills/<name>/SKILL.md`
- Configuração global: `~/.config/openbmw/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o openbmw sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.openbmw/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/openbmw/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.openbmw/skills/git-release/SKILL.md` assim:
...
O openbmw lista as habilidades disponíveis na descrição da ferramenta `skill`.
