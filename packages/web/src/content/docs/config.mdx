...
1. **Remote config** (from `.well-known/openbmw`) - organizational defaults
2. **Global config** (`~/.config/openbmw/opencode\.json`) - user preferences
3. **Custom config** (`OPENCODE_CONFIG` env var) - custom overrides
4. **Project config** (`opencode\.json` in project) - project-specific settings
5. **`.openbmw` directories** - agents, commands, plugins
6. **Inline config** (`OPENCODE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/openbmw/` on macOS) - admin-controlled
...
:::note
The `.openbmw` and `~/.config/openbmw` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/openbmw` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
Place your global OpenCode config in `~/.config/openbmw/opencode\.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/openbmw/tui.json`.
...
modes, and plugins just like the standard `.openbmw` directory, and should
...
The custom directory is loaded after the global config and `.openbmw` directories, so it **can override** their settings.
...
| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/openbmw/` |
| Linux    | `/etc/openbmw/`                         |
| Windows  | `%ProgramData%\openbmw`                 |
...
<dict>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.openbmw</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
You can configure server settings for the `openbmw serve` and `openbmw web` commands through the `server` option.
...
You can also define agents using markdown files in `~/.config/openbmw/agents/` or `.openbmw/agents/`. [Learn more here](/docs/agents).
...
This setting applies across all interfaces: TUI, CLI (`openbmw run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/openbmw/commands/` or `.openbmw/commands/`. [Learn more here](/docs/commands).
...
By default, openbmw **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
Place plugin files in `.openbmw/plugins/` or `~/.config/openbmw/plugins/`. You can also load plugins from npm through the `plugin` option.
