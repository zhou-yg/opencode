...
By default, OpenCode uses our own `openbmw` theme.
...
2. **User config directory** - Defined in `~/.config/openbmw/themes/*.json` or `$XDG_CONFIG_HOME/openbmw/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.openbmw/themes/*.json`
4. **Current working directory** - Defined in `./.openbmw/themes/*.json`
