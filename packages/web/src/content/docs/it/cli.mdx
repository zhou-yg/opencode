...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `openbmw`) |
...
OpenCode si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `openbmw auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/openbmw/auth.json`.
...
Esegui openbmw in modalità non interattiva passando un prompt direttamente.
...
Puoi anche collegarti a una istanza `openbmw serve` già in esecuzione per evitare i cold boot dei server MCP ad ogni esecuzione:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Attach a un server openbmw in esecuzione (es. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `openbmw`) |
...
Avvia un server HTTP che espone accesso API alle funzionalità di openbmw senza la TUI. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `openbmw`).
...
Avvia un server HTTP e apre un browser per accedere a OpenCode tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `openbmw`).
...
Aggiorna openbmw all'ultima versione o a una versione specifica.
...
La CLI di openbmw accetta i seguenti flag globali.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `openbmw`)     |
