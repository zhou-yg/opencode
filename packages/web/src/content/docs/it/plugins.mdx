...
- `.openbmw/plugins/` - plugin a livello progetto
- `~/.config/openbmw/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/openbmw/node_modules/`.
...
1. Config globale (`~/.config/openbmw/opencode\.json`)
2. Config di progetto (`opencode\.json`)
3. Directory plugin globale (`~/.config/openbmw/plugins/`)
4. Directory plugin di progetto (`.openbmw/plugins/`)
...
- `client`: un client SDK di openbmw per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere openbmw.
...
Impedisci a openbmw di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a openbmw:
...
L'helper `tool` crea uno strumento personalizzato che openbmw può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in openbmw insieme agli strumenti integrati.
