...
in `~/.local/share/openbmw/auth.json`.
...
1. Esegui `/connect` nella TUI, seleziona openbmw e vai su [opencode\.ai/auth](https://opencode\.ai/auth).
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui openbmw:
...
Puoi configurare openbmw per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché openbmw funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di openbmw.
...
salvare il token nello storage di auth di openbmw.
:::
...
Per usare il tuo abbonamento GitHub Copilot con openbmw:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui openbmw.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da openbmw, puoi sempre configurarlo tu stesso.
...
Puoi configurare openbmw per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare openbmw per usare modelli locali tramite LM Studio.
...
Puoi configurare openbmw per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per OpenCode. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/openbmw) per dettagli.
:::
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di openbmw.
...
5. Puoi anche personalizzarli tramite la configurazione di openbmw. Ecco un esempio di specifica di un provider
...
Puoi anche personalizzare i modelli tramite la tua configurazione di openbmw. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con openbmw. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
1. **Controlla il setup auth**: Esegui `openbmw auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di openbmw e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config openbmw.
