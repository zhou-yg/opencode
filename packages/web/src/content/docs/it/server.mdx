---
title: Server
description: Interagisci con il server openbmw via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `openbmw serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client openbmw.
...
Imposta `OPENCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `openbmw`, oppure imposta `OPENCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `openbmw serve` sia per `openbmw web`.
...
Quando esegui `openbmw` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server openbmw per interagire con openbmw in modo programmatico.
:::

Questa architettura permette a openbmw di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `openbmw serve` per avviare un server standalone. Se la TUI di openbmw e gia in esecuzione, `openbmw serve` avviera un nuovo server.
...
Il server openbmw espone le seguenti API.
