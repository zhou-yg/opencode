...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\openbmw\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `openbmw --log-level DEBUG`.

---

## Archiviazione

openbmw salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/openbmw/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\openbmw`
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/openbmw/opencode\.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\openbmw\opencode\.jsonc`
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\openbmw\plugins`
...
- **Linux**: elimina `~/.cache/openbmw` (oppure esegui `rm -rf ~/.cache/openbmw`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\openbmw`
...
3. Assicurati di avere l'ultima versione con `openbmw upgrade`
...
Per capire a quali modelli hai accesso, esegui `openbmw models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\openbmw`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. openbmw installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\openbmw`

2. Riavvia openbmw per reinstallare i pacchetti provider piu recenti

Questo forzera openbmw a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
openbmw rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
