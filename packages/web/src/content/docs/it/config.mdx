...
1. **Config remota** (da `.well-known/openbmw`) - default dell'organizzazione
2. **Config globale** (`~/.config/openbmw/opencode\.json`) - preferenze utente
...
5. **Directory `.openbmw`** - agenti, comandi, plugin
...
:::note
Le directory `.openbmw` e `~/.config/openbmw` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Le organizzazioni possono fornire una configurazione predefinita tramite l'endpoint `.well-known/openbmw`. Viene recuperata automaticamente quando ti autentichi con un provider che lo supporta.
...
Metti la configurazione globale di OpenCode in `~/.config/openbmw/opencode\.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.config/openbmw/tui.json`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `OPENCODE_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.openbmw` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.openbmw`, quindi **puo sovrascrivere** le loro impostazioni.
...
Puoi configurare le impostazioni del server per i comandi `openbmw serve` e `openbmw web` tramite l'opzione `server`.
...
Puoi anche definire agenti usando file markdown in `~/.config/openbmw/agents/` o `.openbmw/agents/`. [Scopri di piu](/docs/agents).
...
Questa impostazione si applica a tutte le interfacce: TUI, CLI (`openbmw run`), app desktop e GitHub Action.
...
Puoi anche definire comandi usando file markdown in `~/.config/openbmw/commands/` o `.openbmw/commands/`. [Scopri di piu](/docs/commands).
...
Per impostazione predefinita, openbmw **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
Metti i file dei plugin in `.openbmw/plugins/` o `~/.config/openbmw/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
