...
Per impostazione predefinita, OpenCode usa il tema `openbmw`.
...
2. **Directory di configurazione utente** - in `~/.config/openbmw/themes/*.json` o `$XDG_CONFIG_HOME/openbmw/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.openbmw/themes/*.json`
4. **Directory di lavoro corrente** - in `./.openbmw/themes/*.json`
