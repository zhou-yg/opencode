...
- `.openbmw/plugins/` - Project-level plugins
- `~/.config/openbmw/plugins/` - Global plugins
...
**npm plugins** are installed automatically using Bun at startup. Packages and their dependencies are cached in `~/.cache/openbmw/node_modules/`.
...
1. Global config (`~/.config/openbmw/opencode.json`)
2. Project config (`opencode.json`)
3. Global plugin directory (`~/.config/openbmw/plugins/`)
4. Project plugin directory (`.openbmw/plugins/`)
...
- `client`: An openbmw SDK client for interacting with the AI.
...
Here are some examples of plugins you can use to extend openbmw.
...
Prevent openbmw from reading `.env` files:
...
Plugins can also add custom tools to openbmw:
...
The `tool` helper creates a custom tool that openbmw can call. It takes a Zod schema function and returns a tool definition with:

- `description`: What the tool does
- `args`: Zod schema for the tool's arguments
- `execute`: Function that runs when the tool is called

Your custom tools will be available to openbmw alongside built-in tools.
