---
title: Sağlayıcılar
description: openbmw'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

openbmw, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı openbmw yapılandırmanızda yapılandırın.
...
`~/.local/share/openbmw/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları openbmw'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
OpenCode Zen, openbmw ekibi tarafından sağlanan modellerin bir listesidir.
openbmw ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz OpenCode Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, openbmw'u seçin ve [opencode.ai/auth](https://opencode.ai/auth)'ye gidin.
...
openbmw'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## OpenCode Go

OpenCode Go, openbmw ile iyi çalıştığı test edilmiş ve doğrulanmış, openbmw ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.
...
Amazon Bedrock'u openbmw ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   openbmw'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
...
:::info
Claude Pro/Max aboneliğinizi openbmw'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
openbmw'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   openbmw'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
to store token in openbmw auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
openbmw, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. openbmw'u kilitlemek için
...
GitHub Copilot aboneliğinizi openbmw'la kullanmak için:
...
Google Vertex AI'yi openbmw ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları openbmw çalıştırılırken ayarlayın.
...
Helicone'un openbmw aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her openbmw görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her openbmw görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
openbmw'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla openbmw'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla openbmw'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini openbmw için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/openbmw)'a bakın.
:::
...
Ollama Cloud'u openbmw ile kullanmak için:
...
3. openbmw'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: openbmw'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
OpenCode Zen, openbmw ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).
...
3. openbmw API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   openbmw yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca openbmw yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri openbmw yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı openbmw ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
`limit` alanları openbmw'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `openbmw auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için openbmw yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, openbmw yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
