...
_Model Bağlam Protokolü_ veya MCP kullanarak openbmw'a harici araçlar ekleyebilirsiniz. openbmw hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [openbmw Config](https://opencode\.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/openbmw` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
openbmw, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde openbmw şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa openbmw, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `openbmw mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
```bash
openbmw mcp auth my-oauth-server
```
...
```bash
openbmw mcp logout my-oauth-server
```

`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından openbmw, belirteçleri `~/.local/share/openbmw/mcp-auth.json`'de güvenli bir şekilde saklar.
...
openbmw mcp debug my-oauth-server
```
...
MCP'leriniz yerleşik araçların yanı sıra openbmw'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi openbmw yapılandırması aracılığıyla yönetebilirsiniz.
...
```bash
openbmw mcp auth sentry
```

Bu, OAuth akışını tamamlamak ve openbmw'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
