...
openbmw ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak openbmw kendi `openbmw` temasını kullanır.
...
openbmw birden fazla yerleşik temayla gelir.
...
- openbmw'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
openbmw, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/openbmw/themes/*.json` veya `$XDG_CONFIG_HOME/openbmw/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.openbmw/themes/*.json`
4. **Geçerli çalışma dizini** - `./.openbmw/themes/*.json`
