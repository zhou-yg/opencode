...
-**macOS/리눅스**: `~/.local/share/openbmw/log/`

- **Windows**: 압박 `WIN+R`와 풀 `%USERPROFILE%\.local\share\openbmw\log`

로그 파일은 타임스탬프 (예 : `2025-01-09T123456.log`)과 가장 최근 10 로그 파일이 보관됩니다.

자세한 디버그 정보를 얻기 위해 `--log-level` 명령줄 옵션을 사용하여 로그 레벨을 설정할 수 있습니다. 예를 들면, `openbmw --log-level DEBUG`.

---

## 저장소

openbmw 저장 세션 데이터 및 디스크에 다른 응용 데이터:

-**macOS/리눅스**: `~/.local/share/openbmw/`

- **Windows**: 압박 `WIN+R`와 풀 `%USERPROFILE%\.local\share\openbmw`
...
openbmw 데스크톱은 배경에서 로컬 openbmw 서버 (`opencode-cli` sidecar)를 실행합니다. 대부분의 문제는 misbehaving 플러그인, 손상된 캐시, 또는 나쁜 서버 설정에 의해 발생합니다.
...
-**macOS/리눅스**: `~/.config/openbmw/opencode\.jsonc` (또는 `~/.config/openbmw/opencode\.json`) -**macOS/Linux** (외부 설치): `~/.local/share/openbmw/opencode\.jsonc`

- **Windows**: 압박 `WIN+R`와 풀 `%USERPROFILE%\.config\openbmw\opencode\.jsonc`
...
- **글로벌 플러그인** -**macOS/리눅스**: `~/.config/openbmw/plugins/`
- **Windows**: 압박 `WIN+R`와 풀 `%USERPROFILE%\.config\openbmw\plugins`
...
1. Quit openbmw 데스크톱 완전히.
2. 캐시 디렉토리 삭제:

-**macOS**: 찾기 -> `Cmd+Shift+G` -> 붙여넣기 `~/.cache/openbmw`

- **Linux**: `~/.cache/openbmw` 삭제 (또는 `rm -rf ~/.cache/openbmw` 실행)
- **Windows**: 압박 `WIN+R`와 풀 `%USERPROFILE%\.cache\openbmw`

3. Restart openbmw 데스크탑.

---

## 서버 연결 문제 수정

openbmw 데스크톱은 자체 로컬 서버(기본)를 시작하거나 구성된 서버 URL에 연결할 수 있습니다.
...
Windows에서 openbmw 데스크톱은 Microsoft Edge ** WebView2 실행 시간 **를 요구합니다. 앱이 공백 창에 열거나 시작하지 않을 경우, install/update WebView2를 설치하고 다시 시도하십시오.
...
1. Quit openbmw 데스크탑.
2. 이 파일을 찾아 삭제 (openbmw 데스크톱 앱 데이터 디렉토리에서 라이브):
...
3. 당신은 `openbmw upgrade`를 가진 최신 버전이 있는 것을 지킵니다
...
당신이 액세스 할 수있는 모델을 파악하려면, `openbmw models`를 실행
...
Windows에서, `WIN+R`를 누르고 삭제하십시오: `%USERPROFILE%\.local\share\openbmw`
...
API 호출 오류가 발생하면, 이 공급 업체 패키지로 인해 발생할 수 있습니다. openbmw 동적 설치 공급자 패키지 (OpenAI, Anthropic, Google 등) 필요 하 고 로컬로 캐시.
...
Windows에서, `WIN+R`를 누르고 삭제하십시오: `%USERPROFILE%\.cache\openbmw`

2. 최신 공급자 포장을 재설치하는 Restart openbmw
