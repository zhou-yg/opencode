---
title: CLI
description: openbmw CLI 옵션과 명령어.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

openbmw CLI는 인수 없이 실행하면 기본적으로 [TUI](/docs/tui)를 시작합니다.
...
`serve` 또는 `web` 명령으로 이미 실행 중인 openbmw 백엔드 서버에 터미널을 연결합니다.

```bash
openbmw attach [url]
```

원격 openbmw 백엔드와 TUI를 연결해 사용할 수 있습니다. 예:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | 기본 인증 사용자 이름 (기본값: `OPENCODE_SERVER_USERNAME` 또는 `openbmw`) |
...
OpenCode는 [Models.dev](https://models.dev)의 provider 목록을 기반으로 동작하므로, `openbmw auth login`으로 원하는 provider의 API 키를 설정할 수 있습니다. 인증 정보는 `~/.local/share/openbmw/auth.json`에 저장됩니다.
...
매번 MCP 서버 콜드 부트가 발생하지 않도록, 실행 중인 `openbmw serve` 인스턴스에 붙어서 실행할 수도 있습니다.
...
| <nobr><code>{"--attach"}</code></nobr>   |      | 실행 중인 openbmw 서버에 연결(예: http://localhost:4096)                  |
| <nobr><code>{"--password"}</code></nobr> | `-p` | 기본 인증 비밀번호 (기본값: `OPENCODE_SERVER_PASSWORD`)                    |
| <nobr><code>{"--username"}</code></nobr> | `-u` | 기본 인증 사용자 이름 (기본값: `OPENCODE_SERVER_USERNAME` 또는 `openbmw`) |
...
이 명령은 TUI 없이 openbmw 기능에 접근할 수 있는 HTTP 서버를 시작합니다. `OPENCODE_SERVER_PASSWORD`를 설정하면 HTTP basic auth가 활성화됩니다(기본 사용자명: `openbmw`).
...
이 명령은 HTTP 서버를 시작하고 웹 브라우저를 열어 웹 인터페이스로 OpenCode에 접속합니다. `OPENCODE_SERVER_PASSWORD`를 설정하면 HTTP basic auth가 활성화됩니다(기본 사용자명: `openbmw`).
...
| `OPENCODE_SERVER_USERNAME`            | string  | 기본 인증 사용자명 오버라이드(기본 `openbmw`) |
