...
1. **Remote config**(`.well-known/openbmw`) - 조직 기본값
2. **Global config**(`~/.config/openbmw/opencode\.json`) - 사용자 기본 설정
...
:::note
`.openbmw` 및 `~/.config/openbmw` 디렉토리는 하위 디렉토리에 **복수형 이름**을 사용합니다: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, `themes/`. 단수형 이름(예: `agent/`)도 하위 호환성을 위해 지원합니다.
:::
...
조직은 `.well-known/openbmw` endpoint로 기본 config를 제공할 수 있습니다. 이를 지원하는 provider로 인증하면 자동으로 가져옵니다.
...
전역 OpenCode config는 `~/.config/openbmw/opencode\.json`에 두세요. provider, model, permissions 같은 사용자 전체 기본 설정은 전역 config로 관리하세요.

TUI 관련 설정은 `~/.config/openbmw/tui.json`을 사용하세요.
...
`OPENCODE_CONFIG_DIR` 환경 변수로 custom config 디렉토리를 지정할 수 있습니다. 이 디렉토리는 표준 `.openbmw` 디렉토리와 동일하게 agents, commands, modes, plugins를 검색하며, 동일한 구조를 따라야 합니다.
...
custom 디렉토리는 전역 config와 `.openbmw` 디렉토리 뒤에 로드되므로 해당 설정을 **override할 수 있습니다**.
...
`server` 옵션으로 `openbmw serve`와 `openbmw web` 명령의 server 설정을 구성할 수 있습니다.
...
`~/.config/openbmw/agents/` 또는 `.openbmw/agents/`의 Markdown 파일로 agent를 정의할 수도 있습니다. [더 알아보기](/docs/agents).
...
이 설정은 TUI, CLI(`openbmw run`), 데스크톱 앱, GitHub Action 등 모든 인터페이스에 적용됩니다.
...
`~/.config/openbmw/commands/` 또는 `.openbmw/commands/`의 Markdown 파일로 command를 정의할 수도 있습니다. [더 알아보기](/docs/commands).
...
plugin 파일은 `.openbmw/plugins/` 또는 `~/.config/openbmw/plugins/`에 두세요. `plugin` 옵션으로 npm plugin을 로드할 수도 있습니다.
