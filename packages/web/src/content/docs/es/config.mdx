...
Por ejemplo, si su configuración global establece `theme: "openbmw"` y `autoupdate: true`, y la configuración de su proyecto establece `model: "anthropic/claude-sonnet-4-5"`, la configuración final incluirá las tres configuraciones.
...
1. **Configuración remota** (de `.well-known/openbmw`): valores predeterminados de la organización
2. **Configuración global** (`~/.config/openbmw/opencode.json`) - preferencias del usuario
...
5. Directorios **`.openbmw`** - agentes, comandos, complementos
...
:::note
Los directorios `.openbmw` y `~/.config/openbmw` usan **nombres en plural** para los subdirectorios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` y `themes/`. También se admiten nombres singulares (por ejemplo, `agent/`) para compatibilidad con versiones anteriores.
:::
...
Las organizaciones pueden proporcionar una configuración predeterminada a través del punto final `.well-known/openbmw`. Esto se obtiene automáticamente cuando se autentica con un proveedor que lo admita.
...
Coloque su configuración global OpenCode en `~/.config/openbmw/opencode.json`. Utilice la configuración global para las preferencias de todo el usuario, como temas, proveedores o combinaciones de teclas.
...
modos y complementos como el directorio estándar `.openbmw`, y debería
...
El directorio personalizado se carga después de los directorios global config y `.openbmw`, por lo que **puede anular** su configuración.
...
Puede configurar los ajustes del servidor para los comandos `openbmw serve` y `openbmw web` a través de la opción `server`.
...
También puede definir agentes utilizando archivos de Markdown en `~/.config/openbmw/agents/` o `.openbmw/agents/`. [Más información aquí](/docs/agents).
...
Esta configuración se aplica en todas las interfaces: TUI, CLI (`openbmw run`), aplicación de escritorio y GitHub Acción.
...
También puede definir comandos utilizando archivos de Markdown en `~/.config/openbmw/commands/` o `.openbmw/commands/`. [Más información aquí](/docs/commands).
...
De forma predeterminada, openbmw **permite todas las operaciones** sin requerir aprobación explícita. Puede cambiar esto usando la opción `permission`.
...
Coloque los archivos de complemento en `.openbmw/plugins/` o `~/.config/openbmw/plugins/`. También puedes cargar complementos desde npm a través de la opción `plugin`.
