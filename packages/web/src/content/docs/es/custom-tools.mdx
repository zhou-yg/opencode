...
description: Cree herramientas que LLM pueda llamar en openbmw.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de openbmw como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.openbmw/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/openbmw/tools/`.
...
```ts title=".openbmw/tools/database.ts" {1}
...
```ts title=".openbmw/tools/math.ts"
import { tool } from "@opencode-ai/plugin"
...
```ts title=".openbmw/tools/bash.ts"
import { tool } from "@opencode-ai/plugin"
...
```ts title=".openbmw/tools/project.ts" {8}
import { tool } from "@opencode-ai/plugin"
...
```python title=".openbmw/tools/add.py"
import sys
...
```ts title=".openbmw/tools/python-add.ts" {10}
import { tool } from "@opencode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".openbmw/tools/add.py")
