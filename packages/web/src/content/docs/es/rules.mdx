---
title: Reglas
description: Establezca instrucciones personalizadas para openbmw.
---

Puede proporcionar instrucciones personalizadas a openbmw creando un archivo `AGENTS.md`. Esto es similar a las reglas del Cursor. Contiene instrucciones que se incluirán en el contexto de LLM para personalizar su comportamiento para su proyecto específico.

---

## Inicializar

Para crear un nuevo archivo `AGENTS.md`, puede ejecutar el comando `/init` en openbmw.

:::tip
Debes enviar el archivo `AGENTS.md` de tu proyecto a Git.
:::

Esto escaneará su proyecto y todo su contenido para comprender de qué se trata y generar un archivo `AGENTS.md` con él. Esto ayuda a openbmw a navegar mejor por el proyecto.
...
openbmw también admite la lectura del archivo `AGENTS.md` desde múltiples ubicaciones. Y esto tiene diferentes propósitos.
...
También puedes tener reglas globales en un archivo `~/.config/openbmw/AGENTS.md`. Esto se aplica en todas las sesiones openbmw.
...
- **Reglas globales**: `~/.claude/CLAUDE.md` (se usa si no existe `~/.config/openbmw/AGENTS.md`)
...
Cuando se inicia openbmw, busca archivos de reglas en este orden:

1. **Archivos locales** atravesando desde el directorio actual (`AGENTS.md`, `CLAUDE.md`)
2. **Archivo global** en `~/.config/openbmw/AGENTS.md`
3. **Archivo de Claude Code** en `~/.claude/CLAUDE.md` (a menos que esté deshabilitado)

El primer archivo coincidente gana en cada categoría. Por ejemplo, si tiene `AGENTS.md` y `CLAUDE.md`, solo se usa `AGENTS.md`. De manera similar, `~/.config/openbmw/AGENTS.md` tiene prioridad sobre `~/.claude/CLAUDE.md`.

---

## Instrucciones personalizadas

Puede especificar archivos de instrucciones personalizados en su `opencode\.json` o el `~/.config/openbmw/opencode\.json` global. Esto le permite a usted y a su equipo reutilizar las reglas existentes en lugar de tener que duplicarlas en AGENTS.md.
...
Si bien openbmw no analiza automáticamente las referencias de archivos en `AGENTS.md`, puedes lograr una funcionalidad similar de dos maneras:
...
Puede enseñarle a openbmw a leer archivos externos proporcionando instrucciones explícitas en su `AGENTS.md`. He aquí un ejemplo práctico:
...
- Asegúrese de que openbmw cargue archivos solo cuando sea necesario para la tarea específica
