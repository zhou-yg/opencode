...
1. Ejecute el comando `/connect` en TUI, seleccione openbmw y diríjase a [opencode\.ai/auth](https://opencode\.ai/auth).
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta openbmw:
...
Puedes configurar openbmw para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que openbmw funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración openbmw.
...
para almacenar el token en el almacenamiento de autenticación openbmw.
:::
...
Para utilizar su suscripción GitHub Copilot con openbmw:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta openbmw.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de openbmw, siempre podrá configurarlo usted mismo.
...
Puede configurar openbmw para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar openbmw para usar modelos locales a través de LM Studio.
...
Puedes configurar openbmw para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para OpenCode. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/openbmw) para obtener más detalles.
:::
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración openbmw.
...
5. También puedes personalizarlos a través de tu configuración openbmw. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
También puede personalizar modelos a través de su configuración openbmw. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con openbmw. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
1. **Verifique la configuración de autenticación**: Ejecute `openbmw auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración openbmw y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración openbmw.
