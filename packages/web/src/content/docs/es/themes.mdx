...
De forma predeterminada, OpenCode usa nuestro propio tema `openbmw`.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/openbmw/themes/*.json` o `$XDG_CONFIG_HOME/openbmw/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.openbmw/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.openbmw/themes/*.json`
