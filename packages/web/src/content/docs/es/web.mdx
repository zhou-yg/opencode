...
Para obtener la mejor experiencia, ejecute `openbmw web` desde [WSL](/docs/windows-wsl) en lugar de PowerShell. Esto garantiza el acceso adecuado al sistema de archivos y la integración del terminal.
:::
...
El nombre de usuario predeterminado es `openbmw` pero se puede cambiar con `OPENCODE_SERVER_USERNAME`.
