...
- `.openbmw/plugins/` - Complementos a nivel de proyecto
- `~/.config/openbmw/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/openbmw/node_modules/`.
...
1. Configuración global (`~/.config/openbmw/opencode\.json`)
2. Configuración del proyecto (`opencode\.json`)
3. Directorio global de complementos (`~/.config/openbmw/plugins/`)
4. Directorio de complementos del proyecto (`.openbmw/plugins/`)
...
- `client`: Un cliente SDK openbmw para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar openbmw.
...
Evite que openbmw lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a openbmw:
...
El ayudante `tool` crea una herramienta personalizada a la que openbmw puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para openbmw junto con las herramientas integradas.
