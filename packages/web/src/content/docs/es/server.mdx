---
title: Servidor
description: Interactuar con el servidor openbmw a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `openbmw serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente openbmw puede usar.
...
Configure `OPENCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `openbmw`, o configure `OPENCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `openbmw serve` como a `openbmw web`.
...
Cuando ejecuta `openbmw`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor openbmw para interactuar con openbmw mediante programación.
:::

Esta arquitectura permite que openbmw admita múltiples clientes y le permite interactuar con openbmw mediante programación.

Puede ejecutar `openbmw serve` para iniciar un servidor independiente. Si tienes el
openbmw TUI ejecutándose, `openbmw serve` iniciará un nuevo servidor.
...
El servidor openbmw expone las siguientes API.
