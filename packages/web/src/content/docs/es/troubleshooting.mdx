...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\openbmw\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `openbmw --log-level DEBUG`.

---

## Almacenamiento

openbmw almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/openbmw/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\openbmw`
...
- **macOS/Linux**: `~/.config/openbmw/opencode.jsonc` (o `~/.config/openbmw/opencode.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/openbmw/opencode.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\openbmw\opencode.jsonc`
...
- **Complementos globales**
  ...
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\openbmw\plugins`
...
- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/openbmw`
- **Linux**: eliminar `~/.cache/openbmw` (o ejecutar `rm -rf ~/.cache/openbmw`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\openbmw`
...
3. Asegúrese de tener la última versión con `openbmw upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `openbmw models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\openbmw`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. openbmw instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\openbmw`

2. Reinicie openbmw para reinstalar los paquetes del proveedor más recientes.

Esto obligará a openbmw a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
openbmw detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
