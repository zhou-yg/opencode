---
title: Rules
description: Set custom instructions for openbmw.
---

You can provide custom instructions to openbmw by creating an `AGENTS.md` file. This is similar to Cursor's rules. It contains instructions that will be included in the LLM's context to customize its behavior for your specific project.

---

## Initialize

To create a new `AGENTS.md` file, you can run the `/init` command in openbmw.
...
openbmw also supports reading the `AGENTS.md` file from multiple locations. And this serves different purposes.
...
You can also have global rules in a `~/.config/openbmw/AGENTS.md` file. This gets applied across all openbmw sessions.
...
- **Global rules**: `~/.claude/CLAUDE.md` (used if no `~/.config/openbmw/AGENTS.md` exists)
...
When openbmw starts, it looks for rule files in this order:

1. **Local files** by traversing up from the current directory (`AGENTS.md`, `CLAUDE.md`)
2. **Global file** at `~/.config/openbmw/AGENTS.md`
3. **Claude Code file** at `~/.claude/CLAUDE.md` (unless disabled)

The first matching file wins in each category. For example, if you have both `AGENTS.md` and `CLAUDE.md`, only `AGENTS.md` is used. Similarly, `~/.config/openbmw/AGENTS.md` takes precedence over `~/.claude/CLAUDE.md`.

---

## Custom Instructions

You can specify custom instruction files in your `opencode\.json` or the global `~/.config/openbmw/opencode\.json`. This allows you and your team to reuse existing rules rather than having to duplicate them to AGENTS.md.
...
While openbmw doesn't automatically parse file references in `AGENTS.md`, you can achieve similar functionality in two ways:
...
You can teach openbmw to read external files by providing explicit instructions in your `AGENTS.md`. Here's a practical example:
...
- Ensure openbmw loads files only when needed for the specific task
