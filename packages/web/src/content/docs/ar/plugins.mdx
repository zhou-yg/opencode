...
- `.openbmw/plugins/` - إضافات على مستوى المشروع
- `~/.config/openbmw/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/openbmw/node_modules/`.
...
1. إعدادات عامة (`~/.config/openbmw/opencode.json`)
2. إعدادات المشروع (`opencode.json`)
3. دليل الإضافات العام (`~/.config/openbmw/plugins/`)
4. دليل إضافات المشروع (`.openbmw/plugins/`)
...
- `client`: عميل SDK لـ openbmw للتفاعل مع الذكاء الاصطناعي.
...
فيما يلي بعض أمثلة الإضافات التي يمكنك استخدامها لتوسيع openbmw.
...
امنع openbmw من قراءة ملفات `.env`:
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى openbmw:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ openbmw استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ openbmw إلى جانب الأدوات المضمنة.
