...
1. **شغّل `openbmw web` من terminal داخل WSL** بدلًا من PowerShell:
...
تشغيل `openbmw web` من WSL يضمن وصولًا صحيحًا لنظام الملفات وتكاملًا أفضل مع terminal، مع بقائه متاحًا من متصفح Windows.
...
- إعدادات OpenCode وجلساته تُخزَّن داخل بيئة WSL في `~/.local/share/openbmw/`
