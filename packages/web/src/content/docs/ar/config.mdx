...
على سبيل المثال، إذا كان الإعداد العام يضبط `theme: "openbmw"` و`autoupdate: true`، وكان إعداد المشروع يضبط `model: "anthropic/claude-sonnet-4-5"`، فستتضمن الإعدادات النهائية الخيارات الثلاثة جميعها.
...
1. **الإعدادات البعيدة** (من `.well-known/openbmw`) - الإعدادات الافتراضية على مستوى المؤسسة
2. **الإعدادات العامة** (`~/.config/openbmw/opencode.json`) - تفضيلات المستخدم
3. **إعدادات مخصصة** (`OPENCODE_CONFIG` env var) - تجاوزات مخصصة
4. **إعدادات المشروع** (`opencode.json` داخل المشروع) - إعدادات خاصة بالمشروع
5. **أدلة `.openbmw`** - الوكلاء، الأوامر، الإضافات
...
:::note
يستخدم الدليلان `.openbmw` و`~/.config/openbmw` **أسماء جمع** للأدلة الفرعية: `agents/` و`commands/` و`modes/` و`plugins/` و`skills/` و`tools/` و`themes/`. كما تُدعَم أسماء المفرد (مثل `agent/`) للتوافق مع الإصدارات السابقة.
:::
...
يمكن للمؤسسات توفير إعدادات افتراضية عبر نقطة النهاية `.well-known/openbmw`. يتم جلب ذلك تلقائيًا عندما تقوم بالمصادقة مع مزوّد يدعم هذه الميزة.
...
ضع إعدادات OpenCode العامة في `~/.config/openbmw/opencode.json`. استخدم الإعدادات العامة للتفضيلات على مستوى المستخدم مثل السمات، والمزوّدين، أو اختصارات المفاتيح.
...
الدليل القياسي `.openbmw`، ويجب أن يتبع نفس البنية.
...
يُحمَّل الدليل المخصص بعد الإعدادات العامة وأدلة `.openbmw`، لذلك **يمكنه تجاوز** إعداداتها.
...
يمكنك ضبط إعدادات الخادم لأوامر `openbmw serve` و`openbmw web` عبر الخيار `server`.
...
يمكنك أيضًا تعريف الوكلاء باستخدام ملفات markdown في `~/.config/openbmw/agents/` أو `.openbmw/agents/`. [تعرف على المزيد هنا](/docs/agents).
...
ينطبق هذا الإعداد على جميع الواجهات: TUI وCLI (`openbmw run`) وتطبيق سطح المكتب وGitHub Action.
...
يمكنك أيضًا تعريف الأوامر باستخدام ملفات markdown في `~/.config/openbmw/commands/` أو `.openbmw/commands/`. [تعرف على المزيد هنا](/docs/commands).
...
افتراضيًا يسمح openbmw **بجميع العمليات** دون الحاجة إلى موافقة صريحة. يمكنك تغيير ذلك عبر الخيار `permission`.
...
ضع ملفات الإضافات في `.openbmw/plugins/` أو `~/.config/openbmw/plugins/`. يمكنك أيضًا تحميل إضافات من npm عبر الخيار `plugin`.
