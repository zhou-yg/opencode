...
افتراضيًا، يستخدم OpenCode سمتنا `openbmw`.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/openbmw/themes/*.json` أو `$XDG_CONFIG_HOME/openbmw/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.openbmw/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.openbmw/themes/*.json`
