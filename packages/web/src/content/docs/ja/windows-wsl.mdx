...
1. **PowerShell ではなく WSL ターミナルで `openbmw web` を実行する**
...
WSL から `openbmw web` を実行すると、適切なファイルシステムアクセスとターミナル統合を維持したまま、Windows ブラウザから利用できます。
...
- OpenCode の設定とセッションは WSL 環境内の `~/.local/share/openbmw/` に保存されます
