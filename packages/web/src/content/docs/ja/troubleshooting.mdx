...
- **Windows**: `WIN+R` を押して `%USERPROFILE%\.local\share\openbmw\log` を貼り付けます

ログファイルにはタイムスタンプ付きの名前が付けられ (例: `2025-01-09T123456.log`)、最新の 10 個のログファイルが保存されます。

`--log-level` コマンドラインオプションを使用してログレベルを設定すると、より詳細なデバッグ情報を取得できます。たとえば、`openbmw --log-level DEBUG`。
...
- **Windows**: `WIN+R` を押して `%USERPROFILE%\.local\share\openbmw` を貼り付けます
...
- **macOS/Linux**: `~/.config/openbmw/opencode.jsonc` (または `~/.config/openbmw/opencode.json`)
- **macOS/Linux** (古いインストール): `~/.local/share/openbmw/opencode.jsonc`
- **Windows**: `WIN+R` を押して `%USERPROFILE%\.config\openbmw\opencode.jsonc` を貼り付けます
...
- **グローバルプラグイン**
  ...
  - **Windows**: `WIN+R` を押して `%USERPROFILE%\.config\openbmw\plugins` を貼り付けます
...
- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/openbmw` を貼り付け
- **Linux**: `~/.cache/openbmw` を削除します (または `rm -rf ~/.cache/openbmw` を実行します)。
- **Windows**: `WIN+R` を押して `%USERPROFILE%\.cache\openbmw` を貼り付けます
...
3. `openbmw upgrade` を含む最新バージョンを使用していることを確認してください
...
どのモデルにアクセスできるかを確認するには、`openbmw models` を実行します。
...
2. 問題が解決しない場合は、保存されている構成をクリアしてみてください。
   ...
   Windows では、`WIN+R` を押して、`%USERPROFILE%\.local\share\openbmw` を削除します。
...
1. プロバイダーパッケージのキャッシュをクリアします。
   ...
   Windows では、`WIN+R` を押して、`%USERPROFILE%\.cache\openbmw` を削除します。
