...
- `.openbmw/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/openbmw/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/openbmw/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/openbmw/opencode\.json`)
2. プロジェクト設定 (`opencode\.json`)
3. グローバルプラグインディレクトリ (`~/.config/openbmw/plugins/`)
4. プロジェクトプラグインディレクトリ (`.openbmw/plugins/`)
