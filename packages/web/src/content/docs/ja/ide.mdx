...
OpenCode は、VS Code、Cursor、またはターミナルをサポートする任意の IDE と統合します。開始するには、ターミナルで `openbmw` を実行するだけです。
...
1. VS Code を開く
2. 統合ターミナルを開きます
3. `openbmw` を実行します - 拡張機能は自動的にインストールされます
...
- 統合ターミナルで `openbmw` を実行していることを確認してください。
