...
:::
:::tip[Windows ユーザー]
最高のエクスペリエンスを得るには、PowerShell ではなく [WSL](/docs/windows-wsl) から `openbmw web` を実行します。これにより、ファイルシステムへの適切なアクセスとターミナルの統合が保証されます。
:::
...
ユーザー名のデフォルトは `openbmw` ですが、`OPENCODE_SERVER_USERNAME` で変更できます。
