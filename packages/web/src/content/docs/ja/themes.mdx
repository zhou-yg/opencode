...
デフォルトでは、OpenCode は独自の `openbmw` テーマを使用します。
...
2. **ユーザー設定ディレクトリ** - `~/.config/openbmw/themes/*.json` または `$XDG_CONFIG_HOME/openbmw/themes/*.json` で定義されます
3. **プロジェクトのルートディレクトリ** - `<project-root>/.openbmw/themes/*.json` で定義されます。
4. **現在の作業ディレクトリ** - `./.openbmw/themes/*.json` で定義
...
1. **組み込みテーマ** - これらはバイナリに埋め込まれています
2. **ユーザー設定ディレクトリ** - `~/.config/openbmw/themes/*.json` または `$XDG_CONFIG_HOME/openbmw/themes/*.json` で定義されます
3. **プロジェクトのルート ディレクトリ** - `<project-root>/.openbmw/themes/*.json` で定義されます。
4. **現在の作業ディレクトリ** - `./.openbmw/themes/*.json` で定義
