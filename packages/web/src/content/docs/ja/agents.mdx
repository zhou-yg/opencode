...
- グローバル: `~/.config/openbmw/agents/`
- プロジェクトごと: `.openbmw/agents/`
...
OpenCode 設定内のモデル ID は、`provider/model-id` という形式を使用します。たとえば、[OpenCode Zen](/docs/zen) を使用している場合、GPT 5.1 Codex には `openbmw/gpt-5.1-codex` を使用します。
...
:::tip
`openbmw models` を実行して、利用可能なモデルのリストを表示します。
:::
...
:::tip
共有したいエージェントはいますか? [PR](https://github.com/anomalyco/openbmw) を送信します。
:::
