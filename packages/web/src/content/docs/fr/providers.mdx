...
en `~/.local/share/openbmw/auth.json`.
...
1. Exécutez la commande `/connect` dans le TUI, sélectionnez openbmw et dirigez-vous vers [opencode.ai/auth](https://opencode.ai/auth).
...
Définissez l'une de ces variables d'environnement lors de l'exécution de openbmw :
...
Vous pouvez configurer openbmw pour utiliser des modèles locaux via [Atomic Chat](https://atomic.chat), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `http://127.0.0.1:1337/v1`).
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que openbmw fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration openbmw.
...
pour stocker le jeton dans le stockage d'authentification openbmw.
:::
...
Pour utiliser votre abonnement GitHub Copilot avec openbmw :
...
Définissez-les lors de l'exécution de openbmw.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via openbmw, vous pouvez toujours le configurer vous-même.
...
Vous pouvez configurer openbmw pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer openbmw pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer openbmw pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour OpenCode. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/openbmw) pour plus de détails.
:::
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration openbmw.
...
5. Vous pouvez également les personnaliser via votre configuration openbmw. Voici un exemple de spécification d'un fournisseur
...
Vous pouvez également personnaliser les modèles via votre configuration openbmw. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec openbmw. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
1. **Vérifiez la configuration de l'authentification** : exécutez `openbmw auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration openbmw et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration openbmw.
