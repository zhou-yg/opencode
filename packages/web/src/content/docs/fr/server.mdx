---
title: Serveur
description: Interagissez avec le serveur openbmw via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `openbmw serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client openbmw peut utiliser.
...
Définissez `OPENCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `openbmw`, ou définissez `OPENCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `openbmw serve` et à `openbmw web`.
...
Lorsque vous exécutez `openbmw`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur openbmw pour interagir avec openbmw par programmation.
:::

Cette architecture permet à openbmw de prendre en charge plusieurs clients et vous permet d'interagir avec openbmw par programme.

Vous pouvez exécuter `openbmw serve` pour démarrer un serveur autonome. Si vous avez le
openbmw TUI en cours d'exécution, `openbmw serve` démarrera un nouveau serveur.
...
Le serveur openbmw expose les API suivantes.
