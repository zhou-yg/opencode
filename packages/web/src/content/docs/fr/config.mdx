...
1. **Configuration distante** (à partir de `.well-known/openbmw`) - paramètres par défaut de l'organisation
2. **Configuration globale** (`~/.config/openbmw/opencode\.json`) - préférences utilisateur
...
5. **Répertoires `.openbmw`** - agents, commandes, plugins
...
:::note
Les répertoires `.openbmw` et `~/.config/openbmw` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Les organisations peuvent fournir une configuration par défaut via le point de terminaison `.well-known/openbmw`. Ceci est récupéré automatiquement lorsque vous vous authentifiez auprès d’un fournisseur qui le prend en charge.
...
Placez votre configuration globale OpenCode dans `~/.config/openbmw/opencode\.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.

Pour les paramètres spécifiques à TUI, utilisez `~/.config/openbmw/tui.json`.
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `OPENCODE_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.openbmw`, et devrait suivre la même structure.
...
Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.openbmw`, il **peut donc remplacer** leurs paramètres.
...
Vous pouvez configurer les paramètres du serveur pour les commandes `openbmw serve` et `openbmw web` via l'option `server`.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/openbmw/agents/` ou `.openbmw/agents/`. [En savoir plus ici](/docs/agents).
...
Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`openbmw run`), application de bureau et GitHub Action.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/openbmw/commands/` ou `.openbmw/commands/`. [En savoir plus ici](/docs/commands).
...
Par défaut, openbmw **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
Placez les fichiers du plugin dans `.openbmw/plugins/` ou `~/.config/openbmw/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.
