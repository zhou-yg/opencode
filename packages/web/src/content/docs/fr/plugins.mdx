...
- `.openbmw/plugins/` - Plugins au niveau du projet
- `~/.config/openbmw/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/openbmw/node_modules/`.
...
1. Configuration globale (`~/.config/openbmw/opencode.json`)
2. Configuration du projet (`opencode.json`)
3. Répertoire global des plugins (`~/.config/openbmw/plugins/`)
4. Répertoire des plugins du projet (`.openbmw/plugins/`)
...
- `client` : un client SDK openbmw pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre openbmw.
...
Empêchez openbmw de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à openbmw :
...
L'assistant `tool` crée un outil personnalisé que openbmw peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour openbmw aux côtés des outils intégrés.
