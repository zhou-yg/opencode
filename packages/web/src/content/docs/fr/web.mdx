...
Pour une expérience optimale, exécutez `openbmw web` à partir de [WSL](/docs/windows-wsl) plutôt que de PowerShell. Cela garantit un accès correct au système de fichiers et une intégration du terminal.
:::
...
Le nom d'utilisateur par défaut est `openbmw` mais peut être modifié avec `OPENCODE_SERVER_USERNAME`.
