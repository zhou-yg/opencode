...
OpenCode s'intègre à VS Code, Cursor ou tout IDE prenant en charge un terminal. Exécutez simplement `openbmw` dans le terminal pour commencer.
...
3. Exécutez `openbmw` - l'extension s'installe automatiquement
...
- Assurez-vous que vous exécutez `openbmw` dans le terminal intégré.
