...
![OpenCode TUI avec le thème openbmw](../../../assets/lander/screenshot.png)
...
- **Via Homebrew sur macOS et Linux**
  ...
  > Nous vous recommandons d'utiliser le tap OpenCode pour les versions les plus récentes. La formule officielle `brew install openbmw` est maintenue par l'équipe Homebrew et est mise à jour moins fréquemment.
...
Vous pouvez également récupérer le binaire dans les [Releases](https://github.com/anomalyco/openbmw/releases).
...
1. Exécutez la commande `/connect` dans le TUI, sélectionnez openbmw et allez sur [opencode\.ai/auth](https://opencode\.ai/auth).
