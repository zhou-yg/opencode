...
Par défaut, OpenCode utilise notre propre thème `openbmw`.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/openbmw/themes/*.json` ou `$XDG_CONFIG_HOME/openbmw/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.openbmw/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.openbmw/themes/*.json`
