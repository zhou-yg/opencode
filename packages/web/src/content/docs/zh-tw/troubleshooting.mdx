...
- **Windows**: 按 `WIN+R` 並貼上 `%USERPROFILE%\.local\share\openbmw\log`

日誌檔案以時間戳記命名（例如 `2025-01-09T123456.log`），並保留最近的 10 個日誌檔案。

你可以透過 `--log-level` 命令列選項設定日誌等級以取得更詳細的偵錯資訊。例如：`openbmw --log-level DEBUG`。
...
- **Windows**: 按 `WIN+R` 並貼上 `%USERPROFILE%\.local\share\openbmw`
...
- **macOS/Linux**: `~/.config/openbmw/opencode.jsonc`（或 `~/.config/openbmw/opencode.json`）
- **macOS/Linux**（舊版安裝）: `~/.local/share/openbmw/opencode.jsonc`
- **Windows**: 按 `WIN+R` 並貼上 `%USERPROFILE%\.config\openbmw\opencode.jsonc`
...
- **全域外掛**
  ...
  - **Windows**: 按 `WIN+R` 並貼上 `%USERPROFILE%\.config\openbmw\plugins`
- **專案外掛**（僅當你使用了專案級設定時）
...
- **macOS**: Finder -> `Cmd+Shift+G` -> 貼上 `~/.cache/openbmw`
- **Linux**: 刪除 `~/.cache/openbmw`（或執行 `rm -rf ~/.cache/openbmw`）
- **Windows**: 按 `WIN+R` 並貼上 `%USERPROFILE%\.cache\openbmw`
...
3. 使用 `openbmw upgrade` 確保你使用的是最新版本
...
要查看你有權存取哪些模型，請執行 `openbmw models`
...
2. 如果問題仍然存在，請嘗試清除已儲存的設定：
   ...
   在 Windows 上，按 `WIN+R` 並刪除：`%USERPROFILE%\.local\share\openbmw`
...
1. 清除供應商套件快取：
   ...
   在 Windows 上，按 `WIN+R` 並刪除：`%USERPROFILE%\.cache\openbmw`
