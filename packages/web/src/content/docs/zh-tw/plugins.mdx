...
- `.openbmw/plugins/` - 專案級外掛
- `~/.config/openbmw/plugins/` - 全域外掛
...
**npm 外掛**在啟動時使用 Bun 自動安裝。套件及其相依套件會快取在 `~/.cache/openbmw/node_modules/` 中。
...
1. 全域設定 (`~/.config/openbmw/opencode\.json`)
2. 專案設定 (`opencode\.json`)
3. 全域外掛目錄 (`~/.config/openbmw/plugins/`)
4. 專案外掛目錄 (`.openbmw/plugins/`)
