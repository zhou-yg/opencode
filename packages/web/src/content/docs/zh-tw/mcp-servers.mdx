...
組織可以透過其 `.well-known/openbmw` 端點提供預設的 MCP 伺服器。這些伺服器可能預設處於停用狀態，允許使用者按需啟用。
...
如果伺服器需要身分驗證，OpenCode 會在您首次使用時提示您進行認證。您也可以使用 `openbmw mcp auth <server-name>` [手動觸發認證流程](#authenticating)。
...
`mcp auth` 指令會開啟瀏覽器進行授權。授權完成後，OpenCode 會將 Token 安全地儲存在 `~/.local/share/openbmw/mcp-auth.json` 中。
