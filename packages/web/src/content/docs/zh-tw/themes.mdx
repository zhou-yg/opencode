...
預設情況下，OpenCode 使用我們自己的 `openbmw` 主題。
...
1. **內建主題** — 嵌入在二進位檔案中
2. **使用者設定目錄** — 定義在 `~/.config/openbmw/themes/*.json` 或 `$XDG_CONFIG_HOME/openbmw/themes/*.json`
3. **專案根目錄** — 定義在 `<project-root>/.openbmw/themes/*.json`
4. **當前工作目錄** — 定義在 `./.openbmw/themes/*.json`
