...
1. **在 WSL 終端機中執行 `openbmw web`**，而非在 PowerShell 中執行：
...
從 WSL 中執行 `openbmw web` 可確保正確的檔案系統存取和終端機整合，同時仍可透過 Windows 瀏覽器進行存取。
...
- OpenCode 的設定和工作階段資料儲存在 WSL 環境中的 `~/.local/share/openbmw/`
