...
1. **遠端設定**（來自 `.well-known/openbmw`）- 組織預設值
2. **全域設定**（`~/.config/openbmw/opencode.json`）- 使用者偏好
3. **自訂設定**（`OPENCODE_CONFIG` 環境變數）- 自訂覆寫
4. **專案設定**（專案中的 `opencode.json`）- 專案特定設定
5. **`.openbmw` 目錄** - 代理、指令、外掛程式
6. **內嵌設定**（`OPENCODE_CONFIG_CONTENT` 環境變數）- 執行時覆寫

這意味著專案設定可以覆寫全域預設值，全域設定可以覆寫遠端組織預設值。

:::note
`.openbmw` 和 `~/.config/openbmw` 目錄的子目錄使用**複數名稱**：`agents/`、`commands/`、`modes/`、`plugins/`、`skills/`、`tools/` 和 `themes/`。為了向後相容，也支援單數名稱（例如 `agent/`）。
:::
...
組織可以透過 `.well-known/openbmw` 端點提供預設設定。當您使用支援該功能的供應商進行身分驗證時，會自動擷取此設定。
...
將全域 OpenCode 設定放在 `~/.config/openbmw/opencode.json` 中。使用全域設定來設定使用者層級的偏好，例如主題、供應商或快捷鍵。
...
使用 `OPENCODE_CONFIG_DIR` 環境變數指定自訂設定目錄。該目錄會像標準 `.openbmw` 目錄一樣被搜尋代理、指令、模式和外掛程式，並且應遵循相同的結構。
...
自訂目錄在全域設定和 `.openbmw` 目錄之後載入，因此**可以覆寫**它們的設定。
...
您可以透過 `server` 選項為 `openbmw serve` 和 `openbmw web` 指令設定伺服器設定。
...
您還可以使用 `~/.config/openbmw/agents/` 或 `.openbmw/agents/` 中的 Markdown 檔案定義代理。[在此了解更多](/docs/agents)。
...
此設定適用於所有介面：TUI、CLI（`openbmw run`）、桌面應用程式和 GitHub Action。
...
您還可以使用 `~/.config/openbmw/commands/` 或 `.openbmw/commands/` 中的 Markdown 檔案定義指令。[在此了解更多](/docs/commands)。
...
將外掛程式檔案放置在 `.openbmw/plugins/` 或 `~/.config/openbmw/plugins/` 中。您還可以透過 `plugin` 選項從 npm 載入外掛程式。
