...
- **在 macOS 和 Linux 上使用 Homebrew**
  ...
  > 我們推薦使用 OpenCode tap 以取得最新版本。官方的 `brew install openbmw` formula 由 Homebrew 團隊維護，更新頻率較低。
...
您也可以從 [Releases](https://github.com/anomalyco/openbmw/releases) 頁面直接下載二進位檔案。
...
1. 在 TUI 中執行 `/connect` 指令，選擇 openbmw，然後前往 [opencode\.ai/auth](https://opencode\.ai/auth)。
