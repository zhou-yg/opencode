...
你可以透過 [Atomic Chat](https://atomic.chat) 設定 openbmw 以使用本地模型。Atomic Chat 是一款桌面應用程式，它在 OpenAI 相容的 API 伺服器後方執行本地 LLM（預設端點 `http://127.0.0.1:1337/v1`）。
...
:::tip
Ollama 可以自動為 OpenCode 進行設定。詳見 [Ollama 整合文件](https://docs.ollama.com/integrations/openbmw)。
:::
...
1. **檢查認證設定**：執行 `openbmw auth list` 查看該提供商的憑證是否已新增到設定中。
