...
OpenCode 可以與您的 GitHub 工作流程整合。在評論中提及 `/openbmw` 或 `/oc`，OpenCode 就會在您的 GitHub Actions Runner 中執行任務。
...
| 事件類型                      | 觸發方式                       | 詳情                                                                                      |
| ----------------------------- | ------------------------------ | ----------------------------------------------------------------------------------------- |
| `issue_comment`               | 在 Issue 或 PR 上發表評論      | 在評論中提及 `/openbmw` 或 `/oc`。OpenCode 會讀取上下文，並可建立分支、提交 PR 或回覆。  |
| `pull_request_review_comment` | 在 PR 中對特定程式碼行發表評論 | 在程式碼審查時提及 `/openbmw` 或 `/oc`。OpenCode 會接收檔案路徑、行號和 diff 上下文。    |
