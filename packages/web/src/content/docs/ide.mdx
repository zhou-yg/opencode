...
OpenCode integrates with VS Code, Cursor, or any IDE that supports a terminal. Just run `openbmw` in the terminal to get started.
...
3. Run `openbmw` - the extension installs automatically
...
- Ensure you’re running `openbmw` in the integrated terminal.
