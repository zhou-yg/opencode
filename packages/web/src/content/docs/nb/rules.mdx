...
Du kan også ha globale regler i en `~/.config/openbmw/AGENTS.md`-fil. Dette blir brukt på alle OpenCode-økter.
...
- **Globale regler**: `~/.claude/CLAUDE.md` (brukes hvis ingen `~/.config/openbmw/AGENTS.md` eksisterer)
...
Den første matchende filen vinner i hver kategori. For eksempel, hvis du har både `AGENTS.md` og `CLAUDE.md`, brukes bare `AGENTS.md`. På samme måte har `~/.config/openbmw/AGENTS.md` forrang over `~/.claude/CLAUDE.md`.

---

## Egendefinerte instruksjoner

Du kan spesifisere egendefinerte instruksjonsfiler i din `opencode.json` eller den globale `~/.config/openbmw/opencode.json`. Dette lar deg og teamet ditt gjenbruke eksisterende regler i stedet for å måtte duplisere dem til AGENTS.md.
