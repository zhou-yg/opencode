...
Kommandoen `openbmw serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en OpenCode-klient kan bruke.
...
Sett `OPENCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `openbmw` som standard, eller sett `OPENCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `openbmw serve` og `openbmw web`.
...
Når du kjører `openbmw` starter den en TUI og en server. Der TUI er
...
Du kan kjøre `openbmw serve` for å starte en frittstående server. Hvis du har
OpenCode TUI kjører, vil `openbmw serve` starte en ny server.
