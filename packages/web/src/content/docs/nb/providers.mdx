...
i `~/.local/share/openbmw/auth.json`.
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i openbmw støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere openbmw til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
:::tip
Ollama kan automatisk konfigurere seg selv for OpenCode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/openbmw) for detaljer.
:::
...
1. **Sjekk autentiseringsoppsettet**: Kjør `openbmw auth list` for å se om legitimasjonen
