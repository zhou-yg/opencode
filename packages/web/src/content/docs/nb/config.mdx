---
title: Konfigurasjon
description: Ved å bruke openbmw JSON-konfigurasjonen.
---
...
For eksempel, hvis din globale konfigurasjon setter `theme: "openbmw"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
1. **Ekstern konfigurasjon** (fra `.well-known/openbmw`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/openbmw/opencode\.json`) - brukerinnstillinger
...
5. **`.openbmw` kataloger** - agenter, kommandoer, plugins
...
:::note
`.openbmw` og `~/.config/openbmw` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/openbmw`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale OpenCode-konfigurasjon i `~/.config/openbmw/opencode\.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
moduser og plugins akkurat som standard `.openbmw` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.openbmw`-katalogene, slik at den **kan overstyre** innstillingene deres.
...
Du kan konfigurere serverinnstillinger for kommandoene `openbmw serve` og `openbmw web` gjennom alternativet `server`.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/openbmw/agents/` eller `.openbmw/agents/`. [Les mer her](/docs/agents).
...
Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`openbmw run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/openbmw/commands/` eller `.openbmw/commands/`. [Les mer her](/docs/commands).
...
Plasser plugin-filer i `.openbmw/plugins/` eller `~/.config/openbmw/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
