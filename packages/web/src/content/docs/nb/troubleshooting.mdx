...
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.local\share\openbmw\log`

Loggfiler navngis med tidsstempler (f.eks. `2025-01-09T123456.log`) og de siste 10 loggfilene beholdes.

Du kan angi loggnivået med kommandolinjealternativet `--log-level` for å få mer detaljert feilsøkingsinformasjon. For eksempel `openbmw --log-level DEBUG`.
...
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.local\share\openbmw`
...
- **macOS/Linux** (eldre installasjoner): `~/.local/share/openbmw/opencode.jsonc`
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.config\openbmw\opencode.jsonc`
...
- **Globale plugins**
  ...
  - **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.config\openbmw\plugins`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> lim inn `~/.cache/openbmw`
- **Linux**: slett `~/.cache/openbmw` (eller kjør `rm -rf ~/.cache/openbmw`)
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.cache\openbmw`
...
1. Tøm leverandørens pakkebuffer:
   ...
   På Windows, trykk `WIN+R` og slett: `%USERPROFILE%\.cache\openbmw`
