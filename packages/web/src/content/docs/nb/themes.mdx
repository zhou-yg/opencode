...
Som standard bruker OpenCode vårt eget `openbmw`-tema.
...
2. **Brukerkonfigurasjonskatalog** - Definert i `~/.config/openbmw/themes/*.json` eller `$XDG_CONFIG_HOME/openbmw/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.openbmw/themes/*.json`
4. **Gjeldende arbeidskatalog** - Definert i `./.openbmw/themes/*.json`
