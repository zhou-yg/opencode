...
Der Befehl `openbmw serve` startet einen headless HTTP-Server.
...
Der Benutzername ist standardmaessig `openbmw`, kann aber mit `OPENCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `openbmw serve` und `openbmw web`.
...
Wenn du `openbmw` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den opencode-Server, um openbmw programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `openbmw serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `openbmw serve` trotzdem eine neue Serverinstanz.
