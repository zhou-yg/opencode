---
title: Regeln
description: Lege eigene Anweisungen fuer openbmw fest.
---

Du kannst openbmw ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in openbmw den Befehl `/init` aus.
...
So kann openbmw sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht openbmw sie zu erweitern.
...
openbmw kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/openbmw/AGENTS.md` speichern.
...
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/openbmw/AGENTS.md` existiert)
...
Beim Start sucht openbmw in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/openbmw/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `opencode.json` oder in `~/.config/openbmw/opencode.json` angeben.
...
openbmw parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
...
Du kannst openbmw explizit anweisen, externe Dateien zu lesen.
