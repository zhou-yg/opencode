...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\openbmw\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `openbmw --log-level DEBUG`.

---

## Speicher

openbmw speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/openbmw/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\openbmw`
...
- **macOS/Linux**: `~/.config/openbmw/opencode\.jsonc` (oder `~/.config/openbmw/opencode\.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/openbmw/opencode\.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\openbmw\opencode\.jsonc`
...
- **Globale Plugins**
  ...
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\openbmw\plugins`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/openbmw`
- **Linux**: loesche `~/.cache/openbmw` (oder fuehre aus: `rm -rf ~/.cache/openbmw`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\openbmw`
...
3. Stelle sicher, dass du die neueste Version hast: `openbmw upgrade`
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `openbmw models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\openbmw`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. openbmw installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\openbmw`

2. Starte openbmw neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt openbmw, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
openbmw erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
