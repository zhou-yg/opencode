...
Wenn Ihre globale Konfiguration beispielsweise `theme: "openbmw"` und `autoupdate: true` und Ihre Projektkonfiguration `model: "anthropic/claude-sonnet-4-5"` festlegt, umfasst die endgültige Konfiguration alle drei Einstellungen.
...
1. **Remote-Konfiguration** (von `.well-known/openbmw`) – Organisationsstandards
2. **Globale Konfiguration** (`~/.config/openbmw/opencode\.json`) – Benutzereinstellungen
...
5. **`.openbmw` Verzeichnisse** – Agenten, Befehle, Plugins
...
:::note
Die Verzeichnisse `.openbmw` und `~/.config/openbmw` verwenden **Pluralnamen** für Unterverzeichnisse: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` und `themes/`. Aus Gründen der Abwärtskompatibilität werden auch einzelne Namen (z. B. `agent/`) unterstützt.
:::
...
Organisationen können die Standardkonfiguration über den Endpunkt `.well-known/openbmw` bereitstellen. Dieser wird automatisch abgerufen, wenn Sie sich bei einem Anbieter authentifizieren, der dies unterstützt.
...
Platzieren Sie Ihre globale OpenCode-Konfiguration in `~/.config/openbmw/opencode\.json`. Verwenden Sie die globale Konfiguration für benutzerweite Einstellungen wie Themen, Anbieter oder Tastenkombinationen.
...
Modi und Plugins genau wie das Standardverzeichnis `.openbmw` und sollten
...
Das benutzerdefinierte Verzeichnis wird nach den Verzeichnissen global config und `.openbmw` geladen, sodass es deren Einstellungen **überschreiben** kann.
...
Sie können Servereinstellungen für die Befehle `openbmw serve` und `openbmw web` über die Option `server` konfigurieren.
...
Sie können Agenten auch mithilfe von Markdown-Dateien in `~/.config/openbmw/agents/` oder `.openbmw/agents/` definieren. [Mehr erfahren](/docs/agents).
...
Diese Einstellung gilt für alle Schnittstellen: TUI, CLI (`openbmw run`), Desktop-App und GitHub Aktion.
...
Sie können Befehle auch mithilfe von Markdown-Dateien in `~/.config/openbmw/commands/` oder `.openbmw/commands/` definieren. [Mehr erfahren](/docs/commands).
...
Platzieren Sie Ihre Plugin-Dateien in `.openbmw/plugins/` oder `~/.config/openbmw/plugins/`. Sie können Plugins auch über die Option `plugin` von npm laden.
