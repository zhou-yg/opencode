---
title: LSP-серверы
description: openbmw интегрируется с вашими серверами LSP.
---

openbmw интегрируется с вашим протоколом языкового сервера (LSP), чтобы помочь LLM взаимодействовать с вашей кодовой базой. Он использует диагностику для предоставления обратной связи LLM.

---

## Встроенные

openbmw поставляется с несколькими встроенными LSP-серверами для популярных языков:
...
Когда openbmw открывает файл, он:
...
Вы можете настроить серверы LSP через раздел `lsp` в конфигурации openbmw.
