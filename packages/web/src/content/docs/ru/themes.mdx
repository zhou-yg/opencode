...
С помощью openbmw вы можете выбрать одну из нескольких встроенных тем, использовать тему, которая адаптируется к теме вашего терминала, или определить свою собственную тему.

По умолчанию openbmw использует нашу собственную тему `openbmw`.
...
openbmw поставляется с несколькими встроенными темами.
...
- Хотите, чтобы openbmw соответствовал внешнему виду их терминала
...
openbmw поддерживает гибкую систему тем на основе JSON, которая позволяет пользователям легко создавать и настраивать темы.
...
2. **Каталог конфигурации пользователя** – определяется в `~/.config/openbmw/themes/*.json` или `$XDG_CONFIG_HOME/openbmw/themes/*.json`.
3. **Корневой каталог проекта** – определено в `<project-root>/.openbmw/themes/*.json`.
4. **Текущий рабочий каталог** – определено в `./.openbmw/themes/*.json`.
...
```bash no-frame
mkdir -p ~/.config/openbmw/themes
vim ~/.config/openbmw/themes/my-theme.json
```

Для тем проекта:

```bash no-frame
mkdir -p .openbmw/themes
vim .openbmw/themes/my-theme.json
```
