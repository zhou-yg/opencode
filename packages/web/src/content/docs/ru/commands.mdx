...
Создайте `.openbmw/commands/test.md`:

```md title=".openbmw/commands/test.md"
---
...
Вы можете добавлять собственные команды через конфигурацию openbmw или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем openbmw [config](/docs/config):
...
- Глобальный: `~/.config/openbmw/commands/`
- Для каждого проекта: `.openbmw/commands/`

```markdown title="~/.config/openbmw/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".openbmw/commands/component.md"
---
...
```md title=".openbmw/commands/create-file.md"
---
...
```md title=".openbmw/commands/analyze-coverage.md"
---
...
```md title=".openbmw/commands/review-changes.md"
---
...
```md title=".openbmw/commands/review-component.md"
---
...
openbmw включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
