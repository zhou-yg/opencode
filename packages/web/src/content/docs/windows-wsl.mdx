...
1. **Run `openbmw web` in the WSL terminal** rather than PowerShell:
...
Running `openbmw web` from WSL ensures proper file system access and terminal integration while still being accessible from your Windows browser.
...
- Your OpenCode config and sessions are stored within the WSL environment at `~/.local/share/openbmw/`
