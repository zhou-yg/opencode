...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `OPENCODE_SERVER_USERNAME` eller `openbmw`) |
...
OpenCode drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `openbmw auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/openbmw/auth.json`.
...
Kør openbmw i ikke-interaktiv tilstand ved at sende en prompt direkte.
...
Du kan også vedhæfte til en kørende `openbmw serve`-instans for at undgå MCP serverens kolde opstartstider ved hver kørsel:
...
Dette starter en HTTP-server, der giver API-adgang til opencode-funktionalitet uden TUI-grænsefladen. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `openbmw`).
...
Dette starter en HTTP-server og åbner en webbrowser for at få adgang til OpenCode via en webgrænseflade. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `openbmw`).
...
openbmw CLI tager følgende globale flag.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Tilsidesæt grundlæggende godkendelsesbrugernavn (standard `openbmw`) |
