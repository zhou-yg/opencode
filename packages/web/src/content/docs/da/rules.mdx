---
title: Regler
description: Angiv tilpassede instruktioner til openbmw.
---
...
For at lage en ny `AGENTS.md` fil, kan du kjøre kommandoen `/init` i openbmw.

:::tip
Du bør overgi prosjektets `AGENTS.md`-fil til Git.
:::

Dette vil skanne prosjektet og alt dets innhold for at forstå hva prosjektet handler om og generere en `AGENTS.md`-fil med det. Dette hjelper openbmw at navigere i prosjektet bedre.
...
openbmw støtter også lesing av `AGENTS.md`-filen fra flere steder. Og dette tjener forskjellige formål.
...
Du kan også ha globale regler i en `~/.config/openbmw/AGENTS.md`-fil. Dette blir brugt på alle opencode-sessioner.
...
- **Globale regler**: `~/.claude/CLAUDE.md` (bruges hvis ingen `~/.config/openbmw/AGENTS.md` eksisterer)
...
Når openbmw starter, ser den etter regelfiler i denne rekkefølgen:

1. **Lokale filer** ved at gå opp fra nuværende katalog (`AGENTS.md`, `CLAUDE.md`)
2. **Global fil** på `~/.config/openbmw/AGENTS.md`
3. **Claude Code-fil** på `~/.claude/CLAUDE.md` (med mindre de er deaktivert)

Den første matchende filen vinner i hver kategori. For eksempel, hvis du har både `AGENTS.md` og `CLAUDE.md`, bruges bare `AGENTS.md`. På samme måte har `~/.config/openbmw/AGENTS.md` forrang over `~/.claude/CLAUDE.md`.

---

## Brugerdefinerede instruktioner

Du kan spesifisere egendefinerte instruksjonsfiler i din `opencode\.json` eller den globale `~/.config/openbmw/opencode\.json`. Dette lar deg og teamet ditt gjenbruge eksisterende regler i stedet for at måtte duplisere dem til AGENTS.md.
...
Selv om openbmw ikke automatisk analyserer filreferanser i `AGENTS.md`, kan du oppnå lignende funksjonalitet på to måter:
...
Du kan lære openbmw at lese eksterne filer ved at gi eksplisitte instruksjoner i `AGENTS.md`. Her er et praktisk eksempel:
...
- Sørg for at openbmw laster filer kun når det er nødvendig for den spesifikke oppgaven
