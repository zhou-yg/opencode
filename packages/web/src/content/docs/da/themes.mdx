...
Som standard bruger OpenCode vårt eget `openbmw`-tema.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/openbmw/themes/*.json` eller `$XDG_CONFIG_HOME/openbmw/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.openbmw/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.openbmw/themes/*.json`
