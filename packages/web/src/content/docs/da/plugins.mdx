...
- `.openbmw/plugins/` - Plugins på projektniveau
- `~/.config/openbmw/plugins/` - Globale plugins
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/openbmw/node_modules/`.
...
1. Global konfiguration (`~/.config/openbmw/opencode\.json`)
2. Projektkonfiguration (`opencode\.json`)
3. Global plugin-mappe (`~/.config/openbmw/plugins/`)
4. Projekt plugin bibliotek (`.openbmw/plugins/`)
...
- `client`: En openbmw SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide openbmw.
...
Undgå openbmw i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til openbmw:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som openbmw kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for openbmw sammen med indbyggede værktøjer.
