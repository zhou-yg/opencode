...
1. **Fjernkonfiguration** (fra `.well-known/openbmw`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/openbmw/opencode.json`) - brugerpræferencer
...
5. **`.openbmw` mapper** - agent, kommandoer, plugins
...
:::note
`.openbmw` og `~/.config/openbmw` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/openbmw`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale OpenCode-konfiguration i `~/.config/openbmw/opencode.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
modes og plugins ligesom standard `.openbmw` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.openbmw` mapper, så den **kan tilsidesætte** deres indstillinger.
...
Du kan konfigurere serverindstillinger for kommandoerne `openbmw serve` og `openbmw web` gennem indstillingen `server`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/openbmw/agents/` eller `.openbmw/agents/`. [Learn more here](/docs/agents).
...
Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`openbmw run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/openbmw/commands/` eller `.openbmw/commands/`. [Learn more here](/docs/commands).
...
Som standard openbmw **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
Placer plugin-filer i `.openbmw/plugins/` eller `~/.config/openbmw/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
