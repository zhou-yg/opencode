...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\openbmw\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `openbmw --log-level DEBUG`.

---

## Lagring

openbmw gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/openbmw/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\openbmw`
...
- **macOS/Linux**: `~/.config/openbmw/opencode\.jsonc` (eller `~/.config/openbmw/opencode\.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/openbmw/opencode\.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\openbmw\opencode\.jsonc`
...
- **Globale plugins**
  ...
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\openbmw\plugins`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/openbmw`
- **Linux**: slet `~/.cache/openbmw` (eller kør `rm -rf ~/.cache/openbmw`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\openbmw`
...
3. Sørg for at du har den nyeste version med `openbmw upgrade`
...
For at finde ud af hvilke modeller du har adgang til, kør `openbmw models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\openbmw`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. openbmw installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\openbmw`

2. Start openbmw på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge openbmw til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
openbmw vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
