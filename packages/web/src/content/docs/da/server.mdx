...
Kommandoen `openbmw serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en opencode-klient kan bruge.
...
Indstil `OPENCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `openbmw`, eller indstil `OPENCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `openbmw serve` og `openbmw web`.
...
Når du kører `openbmw` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug opencode-serveren til at interagere med openbmw programmatisk.
:::

Denne arkitektur lader openbmw understøtte flere klienter og lader dig interagere med openbmw programmatisk.

Du kan køre `openbmw serve` for at starte en selvstændig server. Hvis du har
openbmw TUI kørende, vil `openbmw serve` starte en ny server.
