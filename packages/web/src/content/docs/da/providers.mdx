...
Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/openbmw/auth.json`.
...
1. Kør kommandoen `/connect` i TUI, vælg openbmw og gå til [opencode.ai/auth](https://opencode.ai/auth).
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører openbmw:
...
Du kan konfigurere openbmw til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at openbmw skal fungere korrekt.
   :::
...
gemme tokenet i openbmw auth-lagring.
:::
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem openbmw, kan du altid konfigurere den selv.
...
Du kan konfigurere openbmw til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere openbmw til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere openbmw til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for OpenCode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/openbmw) for detaljer.
:::
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med openbmw. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
1. **Tjek godkendelsesopsætningen**: Kør `openbmw auth list` for at se om legitimationsoplysningerne
