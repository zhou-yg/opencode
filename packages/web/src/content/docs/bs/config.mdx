...
1. **Udaljena konfiguracija** (od `.well-known/openbmw`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/openbmw/opencode.json`) - korisničke preferencije
...
5. **`.openbmw` direktoriji** - agenti, komande, dodaci
...
:::note
Direktoriji `.openbmw` i `~/.config/openbmw` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/openbmw` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu OpenCode konfiguraciju u `~/.config/openbmw/opencode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/openbmw/tui.json`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `OPENCODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.openbmw` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.openbmw`, tako da **može nadjačati** njihove postavke.
...
Možete konfigurirati postavke servera za naredbe `openbmw serve` i `openbmw web` putem opcije `server`.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/openbmw/agents/` ili `.openbmw/agents/`. [Saznajte više ovdje](/docs/agents).
...
Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`openbmw run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/openbmw/commands/` ili `.openbmw/commands/`. [Saznajte više ovdje](/docs/commands).
...
Postavite datoteke dodataka u `.openbmw/plugins/` ili `~/.config/openbmw/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
