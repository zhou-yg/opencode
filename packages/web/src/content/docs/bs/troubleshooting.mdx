...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\openbmw\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `openbmw --log-level DEBUG`.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\openbmw`
...
- **macOS/Linux**: `~/.config/openbmw/opencode.jsonc` (ili `~/.config/openbmw/opencode.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/openbmw/opencode.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\openbmw\opencode.jsonc`
...
- **Globalni dodaci**
  ...
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\openbmw\plugins`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/openbmw`
- **Linux**: obrišite `~/.cache/openbmw` (ili pokrenite `rm -rf ~/.cache/openbmw`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\openbmw`
...
3. Uvjerite se da imate najnoviju verziju sa `openbmw upgrade`
...
Da saznate kojim modelima imate pristup, pokrenite `openbmw models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\openbmw`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\openbmw`
