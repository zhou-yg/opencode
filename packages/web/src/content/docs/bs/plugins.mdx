...
- `.openbmw/plugins/` - Dodaci na nivou projekta
- `~/.config/openbmw/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/openbmw/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/openbmw/opencode\.json`)
2. Konfiguracija projekta (`opencode\.json`)
3. Globalni direktorij dodataka (`~/.config/openbmw/plugins/`)
4. Direktorij dodataka projekta (`.openbmw/plugins/`)
...
Spriječite openbmw da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u openbmw:
...
Pomoćnik `tool` kreira prilagođeni alat koji openbmw može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za openbmw zajedno sa ugrađenim alatima.
