...
Po defaultu, OpenCode koristi nasu `openbmw` temu.
...
2. **Korisnicki config direktorij** - `~/.config/openbmw/themes/*.json` ili `$XDG_CONFIG_HOME/openbmw/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.openbmw/themes/*.json`
4. **Trenutni radni direktorij** - `./.openbmw/themes/*.json`
