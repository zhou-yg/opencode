---
title: Server
description: Komunicirajte s openbmw serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `openbmw serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji openbmw klijent moze koristiti.
...
Postavite `OPENCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `openbmw`, ili postavite `OPENCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `openbmw serve` i za `openbmw web`.
...
Kada pokrenete `openbmw`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite openbmw server za programsku interakciju sa opencode-om.
:::

Ova arhitektura omogucava openbmw podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `openbmw serve` da startate standalone server. Ako je openbmw TUI vec pokrenut, `openbmw serve` ce pokrenuti novi server.
...
openbmw server izlaže sljedece API-je.
