---
title: Server
description: Interact with openbmw server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `openbmw serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an openbmw client can use.
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `openbmw`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `openbmw serve` and `openbmw web`.
...
When you run `openbmw` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the openbmw server to interact with openbmw programmatically.
:::

This architecture lets openbmw support multiple clients and allows you to interact with openbmw programmatically.

You can run `openbmw serve` to start a standalone server. If you have the
openbmw TUI running, `openbmw serve` will start a new server.
...
The openbmw server exposes the following APIs.
