...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running openbmw:
...
You can configure openbmw to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for openbmw to work properly.
   :::
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your openbmw config.
...
To use your GitHub Copilot subscription with openbmw:
...
2. Set the required environment variables:
   ...
   Set them while running openbmw.
...
In the event you see a feature or model from Helicone that isn't configured automatically through openbmw, you can always configure it yourself.
...
You can configure openbmw to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure openbmw to use local models through LM Studio.
...
You can configure openbmw to use local models through Ollama.

:::tip
Ollama can automatically configure itself for OpenCode. See the [Ollama integration docs](https://docs.ollama.com/integrations/openbmw) for details.
:::
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your openbmw config.
...
5. You can also customize them through your openbmw config. Here's an example of specifying a provider
...
You can also customize models through your openbmw config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with openbmw. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
1. **Check the auth setup**: Run `openbmw auth list` to see if the credentials
...
2. For custom providers, check the openbmw config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your openbmw config.
