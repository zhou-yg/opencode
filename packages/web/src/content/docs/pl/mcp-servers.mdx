...
Możesz dodać dodatkowe informacje do openbmw za pomocą _Model Context Protocol_, w skrócie MCP. openbmw obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [openbmw Config](https://opencode\.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/openbmw`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
openbmw automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, openbmw:
...
Jeśli serwer wymaga uwierzytelnienia, openbmw poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `openbmw mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji openbmw bezpieczne przechowa tokeny w `~/.local/share/openbmw/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w openbmw, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie openbmw, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć openbmw z kontem Sentry.
