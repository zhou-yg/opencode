---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć openbmw.
---

Wtyczki umożliwiające wyłączenie openbmw poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania openbmw.
...
- `.openbmw/plugins/` - Wtyczki na poziomie projektu
- `~/.config/openbmw/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/openbmw/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/openbmw/opencode\.json`)
2. Konfiguracja projektu (`opencode\.json`)
3. Globalny katalog wtyczek (`~/.config/openbmw/plugins/`)
4. Katalog wtyczek projektu (`.openbmw/plugins/`)
...
openbmw uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z openbmw do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do openbmw.
...
:::note
Jeśli korzystasz z aplikacji komputerowej openbmw, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj openbmw czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do openbmw:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod openbmw. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
