...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "openbmw"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/openbmw`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/openbmw/opencode\.json`) – preferencje użytkownika
...
5. **Katalogi `.openbmw`** - agenci, polecenia, umiejętności
...
:::note
W katalogach `.openbmw` i `~/.config/openbmw` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/openbmw`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację OpenCode w `~/.config/openbmw/opencode\.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/openbmw/tui.json`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `OPENCODE_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.openbmw` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.openbmw`, więc **może być łączony** z ich zawartością.
...
Możesz skonfigurować serwer dla `openbmw serve` i `openbmw web` za pomocą opcji `server`.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/openbmw/agents/` lub `.openbmw/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Dotyczy to wszystkich interfejsów: TUI, CLI (`openbmw run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/openbmw/commands/` lub `.openbmw/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
Umieść pliki wtyczek w `.openbmw/plugins/` lub `~/.config/openbmw/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
