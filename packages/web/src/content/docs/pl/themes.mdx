...
Dzięki openbmw możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie openbmw używa naszego własnego motywu `openbmw`.
...
openbmw ma kilka wbudowanych motywów.
...
openbmw obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/openbmw/themes/*.json` lub `$XDG_CONFIG_HOME/openbmw/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.openbmw/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.openbmw/themes/*.json`
