...
Aby debugować problemy z openbmw, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\openbmw\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `openbmw --log-level DEBUG`.

---

## Przechowywanie danych

openbmw przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/openbmw/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\openbmw`
...
openbmw Desktop uruchamia w tle lokalny serwer openbmw (sidecar `opencode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- **macOS/Linux**: `~/.config/openbmw/opencode\.jsonc` (lub `~/.config/openbmw/opencode\.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/openbmw/opencode\.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\openbmw\opencode\.jsonc`
...
openbmw może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/openbmw/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\openbmw\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby openbmw mógł ją odbudować.

1. Całkowicie zamknij openbmw Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/openbmw`
- **Linux**: usuń `~/.cache/openbmw` (lub uruchom `rm -rf ~/.cache/openbmw`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\openbmw`

3. Uruchom ponownie openbmw Desktop.

---

### Napraw problemy z połączeniem z serwerem

openbmw Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
W systemie Windows openbmw Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami openbmw.

---

### Brak powiadomień

openbmw Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla openbmw w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie openbmw Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji openbmw Desktop):
...
3. Upewnij się, że masz najnowszą wersję z `openbmw upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `openbmw models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\openbmw`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. openbmw dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\openbmw`

2. Uruchom ponownie kod openbmw, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to openbmw do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
openbmw wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
