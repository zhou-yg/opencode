...
Narzędzia specjalistyczne to funkcje, które tworzysz i które LLM może wywoływać podczas rozmów. Współpracują one z [wbudowanymi narzędziami](/docs/tools) openbmw, takimi jak `read`, `write` i `bash`.
...
- Lokalnie, umieszczając je w katalogu `.openbmw/tools/` swojego projektu.
- Lub globalnie, umieszczając je w `~/.config/openbmw/tools/`.
