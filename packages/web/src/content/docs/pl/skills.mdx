...
Umiejętności agenta pozwalają openbmw odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
openbmw przeszukuje te lokalizacje:

- Project config: `.openbmw/skills/<name>/SKILL.md`
- Global config: `~/.config/openbmw/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu openbmw przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.openbmw/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/openbmw/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.openbmw/skills/git-release/SKILL.md` w ten sposób:
...
openbmw wymienia dostępne umiejętności w opisie narzędzia `skill`.
