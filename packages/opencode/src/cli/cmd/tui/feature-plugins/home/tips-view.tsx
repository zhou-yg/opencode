...
const TIPS = [
  ...
  "Place TUI settings in {highlight}~/.config/openbmw/tui.json{/highlight} for global config",
  ...
  "Add {highlight}.md{/highlight} files to {highlight}.bmw/command/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.bmw/agent/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.bmw/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.bmw/plugin/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent OpenCode from reading sensitive files",
  "Use {highlight}openbmw run{/highlight} for non-interactive scripting",
  "Use {highlight}openbmw --continue{/highlight} to resume the last session",
  "Use {highlight}openbmw run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}openbmw serve{/highlight} for headless API access to OpenCode",
  "Use {highlight}openbmw run --attach{/highlight} to connect to a running server",
  "Run {highlight}openbmw upgrade{/highlight} to update to the latest version",
  "Run {highlight}openbmw auth list{/highlight} to see all configured providers",
  "Run {highlight}openbmw agent create{/highlight} for guided agent creation",
  "Use {highlight}/openbmw{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}openbmw github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/openbmw fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.bmw/themes/{/highlight} directory",
  ...
  "Run {highlight}openbmw debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/anomalyco/openbmw{/highlight} for containerized use",
