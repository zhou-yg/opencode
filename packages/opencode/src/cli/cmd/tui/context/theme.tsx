...
import openbmw from "./theme/opencode\.json" with { type: "json" }
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "openbmw")
        draft.active = typeof active === "string" ? active : "openbmw"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.openbmw, store.mode)
    })
