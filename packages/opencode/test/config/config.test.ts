...
test("ignores legacy tui keys in openbmw config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://opencode\.ai/config.json",
...
test("resolves env templates in account config with account token", async () => {
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { openbmw: { options: { apiKey: "{env:OPENCODE_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["openbmw"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
...
test("loads config from .openbmw directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".openbmw")
...
test("agent markdown permission config preserves user key order", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentDir = path.join(dir, ".openbmw", "agent")
      await fs.mkdir(agentDir, { recursive: true })
...
test("loads agents from .openbmw/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".openbmw")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .openbmw/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".openbmw")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .openbmw/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".openbmw")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .openbmw config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".openbmw")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .openbmw config with different plugins
      await Filesystem.write(
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .openbmw config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "opencode\.json"),
        JSON.stringify({
          $schema: "https://opencode\.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".openbmw")
...
test("local .openbmw config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .openbmw directory config enables it
      const opencodeDir = path.join(dir, ".openbmw")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "opencode\.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/openbmw")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/openbmw")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    ...
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-openbmw@2.4.3", file)).toBe("oh-my-openbmw@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-openbmw@2.4.3", "file:///project/.openbmw/plugin/oh-my-opencode\.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.openbmw/plugin/demo.ts", "file:///project/.openbmw/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.openbmw/plugin/demo.ts"])
  })
...
describe("OPENCODE_DISABLE_PROJECT_CONFIG", () => {
  ...
  test("skips project .openbmw/ directories when flag is set", async () => {
    const originalEnv = process.env["OPENCODE_DISABLE_PROJECT_CONFIG"]
    process.env["OPENCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .openbmw directory with a command
          const opencodeDir = path.join(dir, ".openbmw", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await WithInstance.provide({
        ...
        fn: async () => {
          ...
          // Project .openbmw should NOT be in directories list
