...
test("plan agent denies edits except .openbmw/plans/*", async () => {
  await using tmp = await tmpdir()
  await WithInstance.provide({
    directory: tmp.path,
    fn: async () => {
      ...
      expect(Permission.evaluate("edit", ".openbmw/plans/foo.md", plan!.permission).action).toBe("allow")
    },
  })
})
...
test("skill directories are allowed for external_directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".openbmw", "skill", "perm-skill")
      await Bun.write(
...
`,
  ...
  try {
    await WithInstance.provide({
      ...
      fn: async () => {
        const build = await load(tmp.path, (svc) => svc.get("build"))
        const skillDir = path.join(tmp.path, ".openbmw", "skill", "perm-skill")
