...
function paid(providers: Awaited<ReturnType<typeof list>>) {
  const item = providers[ProviderID.make("openbmw")]
        ...
        JSON.stringify({
          ...
          provider: {
            "cloudflare-ai-gateway": {
              options: {
                metadata: { invoked_by: "test", project: "openbmw" },
              },
            },
          },
        }),
...
test("plugin config providers persist after instance dispose", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const configDir = path.join(dir, ".openbmw")
...
test("plugin config enabled and disabled providers are honored", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const root = path.join(dir, ".openbmw", "plugin")
      await mkdir(root, { recursive: true })
      await Bun.write(
...
test("openbmw loader keeps paid models when config apiKey is present", async () => {
...
test("openbmw loader keeps paid models when auth exists", async () => {
  await using base = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "opencode.json"),
