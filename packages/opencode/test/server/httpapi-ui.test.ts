...
describe("HttpApi UI fallback", () => {
  test("serves the web UI through the experimental backend", async () => {
    ...
    const response = await uiApp({
      client: httpClient(
        new Response("<html>openbmw</html>", { headers: { "content-type": "text/html" } }),
        (request) => {
    ...
    expect(await response.text()).toBe("<html>openbmw</html>")
            ...
            Layer.succeed(
              ...
              HttpClient.make((request) =>
                Effect.succeed(
                  HttpClientResponse.fromWeb(
                    request,
                    new Response("<html>openbmw</html>", {
                      headers: {
  ...
  test("requires server password for the web UI", async () => {
    ...
    const response = await uiApp({ password: "secret", username: "openbmw" }).request("/")
  ...
  test("accepts auth token for the web UI", async () => {
    ...
    const response = await uiApp({
      password: "secret",
      username: "openbmw",
      client: httpClient(new Response("<html>openbmw</html>", { headers: { "content-type": "text/html" } })),
    }).request(`/?auth_token=${btoa("openbmw:secret")}`)
  ...
  test("accepts basic auth for the web UI", async () => {
    ...
    const response = await uiApp({ password: "secret", username: "openbmw" }).request("/", {
      headers: { authorization: `Basic ${btoa("openbmw:secret")}` },
    })
  ...
  test("allows web UI preflight without auth", async () => {
    ...
    const response = await app({ password: "secret", username: "openbmw" }).request("/", {
      method: "OPTIONS",
      headers: {
